...
class Ho extends CI_Controller {
	...
	public function fo_daftar($jenis_izin=1,$idpelanggan=0,$idperusahaan=0)
	{
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Access'] = 'fo';
		$d['Kd_Izin'] = $this->kode_izin; //kode ho
		$d['ID_Izin'] = 0;
	...
	public function fo_edit($id=0)
	{
		...
		$d['op_bentuk_perusahaan'] = $this->mod_ho->op_bentuk_perusahaan();
		$d['data'] = $this->mod_ho->get_personal($idpelanggan);
		$d['data2'] = $this->mod_ho->get_data_ho($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(98),$DataAkses);
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_ho->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_ho->get_kategori_izin($id);
		$d['data'] = $this->mod_ho->get_ho($id);
		$d['Kd_Izin'] = $this->kode_izin; //kode ho
		$d['Nm_Izin'] = $this->nama_izin; //kode ho
	...
	public function sk($id=0)
	{
		...
		switch($jenis){
			case 1 :
				...
				break;
			case 2 :
				$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk_balik',$d);
				break;
			case 3 :
				$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk_alih',$d);
				break;
			default:
				$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk',$d);
				break;
	...
	public function sk2($id=0)
	{
		...
		switch($jenis){
			case 1 :
				...
				break;
			case 2 :
				$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk2_balik',$d);
				break;
			case 3 :
				$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk2_alih',$d);
				break;
			default:
				$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk2',$d);
				break;
