...
class Siup extends CI_Controller {
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(58,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_siup->get_tbl_siup_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_siup->get_tbl_siup_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_siup->get_tbl_siup_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_siup->get_tbl_siup_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_daftar($jenis_izin=1,$idpelanggan=0,$idperusahaan=0)
	{
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Access'] = 'fo';
		$d['Kd_Izin'] = $this->kode_izin; //kode siup
		$d['ID_Izin'] = 0;
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_siup->get_data_siup($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		$d['op_bentuk_perusahaan'] = $this->mod_siup->op_bentuk_perusahaan();
		$d['data'] = $this->mod_siup->get_personal($idpelanggan);
		$d['data2'] = $this->mod_siup->get_data_siup($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
		...
		$d['op_serah_terima'] = $this->mod_siup->op_serah_terima();
		$d['op_bukti_terima'] = $this->mod_siup->op_bukti_terima();
		$d['replace'] = $this->mod_siup->for_replace($id);
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(58),$DataAkses);
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_siup->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_siup->get_kategori_izin($id);
		$d['data'] = $this->mod_siup->get_siup($id);
		$d['Kd_Izin'] = $this->kode_izin; //kode siup
		$d['Nm_Izin'] = $this->nama_izin; //kode siup
