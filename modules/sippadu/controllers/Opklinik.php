...
class Opklinik extends CI_Controller {
	...
	var $nama_izin 	= 'Izin Mendirikan dan Operasional Klinik';
	...
	public function fo($offset=0)
	{
		...
		$d['PageAkses'] = PageAkses(180,$DataAkses);
		
		if($this->input->post('delete')){
			$this->mod_opklinik->delete_tbl_opklinik();
		}
		...
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_fo(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_fo(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function fo_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_fo(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_fo(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function bo($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(181,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_bo(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_bo(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function bo_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_bo(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_bo(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function kabid($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(182,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('kabid',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_kabid(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_kabid(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function kabid_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('kabid',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_kabid(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_kabid(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function kadin($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(183,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('kadin',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_kadin(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_kadin(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function kadin_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('kadin',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_kadin(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_kadin(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function tu($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(184,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('tu',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_tu(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_tu(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function tu_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('tu',0);
		}
		$d['title'] = $title = $this->nama_izin;
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'tu';
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_tu(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_tu(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function cetak($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(185,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_cetak(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_cetak(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function cetak_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_cetak(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_cetak(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function serah($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(186,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('serah',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_serah(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_serah(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function serah_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_opklinik->verifikasi_tbl_opklinik('serah',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_serah(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_serah(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function serah_win($idizin=0)
	{
		...
		if($this->input->post('submit')){
			$this->mod_opklinik->simpan_pengambil($idizin);
		}
		...
		$d['data'] = $this->mod_opklinik->data_serah($idizin);
	...
	{
		$d['data'] = $this->mod_opklinik->data_serah_ajax();
		$d['Class_Izin'] = $this->class_izin;
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(188,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_opklinik_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_opklinik_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_data($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_opklinik->get_tbl_personal($limit,$offset);
		$d['tot'] = $tot = $this->mod_opklinik->get_tbl_personal()->num_rows();
	...
	public function fo_daftar($jenis_izin=1,$idpelanggan=0,$idperusahaan=0)
	{
		...
		if($this->input->post('submit')){
			$this->mod_opklinik->insert_opklinik_fo();
		}
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Access'] = 'fo';
		$d['Kd_Izin'] = $this->kode_izin; //kode opklinik
		$d['ID_Izin'] = 0;
		$d['op_provinsi'] = $this->mod_opklinik->op_provinsi();
		$d['op_kota'] = $this->mod_opklinik->op_kota();
		$d['op_kecamatan'] = $this->mod_opklinik->op_kecamatan();
		$d['op_desa'] = $this->mod_opklinik->op_desa();
		$d['op_perusahaan'] = $this->mod_opklinik->op_perusahaan($idpelanggan);
		$d['op_bentuk_perusahaan'] = $this->mod_opklinik->op_bentuk_perusahaan();
		$d['data'] = $this->mod_opklinik->get_personal($idpelanggan);
		...
		$d['op_status_perusahaan'] = $this->mod_opklinik->op_status_perusahaan();
		$d['op_kategori_opklinik'] = $this->mod_opklinik->op_kategori_opklinik();
		$d['op_kegiatan_usaha'] = $this->mod_opklinik->op_kegiatan_usaha();
		$d['op_kelembagaan'] = $this->mod_opklinik->op_kelembagaan();
		$d['op_verifikasi'] = $this->mod_opklinik->op_verifikasi();
		$d['op_serah_terima'] = $this->mod_opklinik->op_serah_terima();
		$d['op_bukti_terima'] = $this->mod_opklinik->op_bukti_terima();
		$d['op_kategori_izin'] = $this->mod_opklinik->op_kategori_izin();
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_opklinik->get_opklinik($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		if($this->input->post('submit')){
			$this->mod_opklinik->update_opklinik($id);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_opklinik->op_bentuk_perusahaan();
		$d['data'] = $this->mod_opklinik->get_personal($idpelanggan);
		$d['data2'] = $this->mod_opklinik->get_data_opklinik($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
		...
		$d['op_bukti_terima'] = $this->mod_opklinik->op_bukti_terima();
		$d['op_kategori_izin'] = $this->mod_opklinik->op_kategori_izin();
		$d['replace'] = $this->mod_opklinik->for_replace($id);
	...
	public function bo_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(180,181),$DataAkses);
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(188),$DataAkses);
	...
	public function fo_proses($id=0,$jenis_izin=0,$idpelanggan=0,$idperusahaan=0)
	{
		...
		if($this->input->post('submit')){
			...
			$this->mod_opklinik->update_opklinik_fo($idperusahaan);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_opklinik->op_bentuk_perusahaan();
		$d['data'] = $this->mod_opklinik->get_personal($idpelanggan);
		$d['data_p'] = $this->mod_opklinik->get_opklinik($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Izin'] = $this->kode_izin; //kode opklinik
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_opklinik->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_opklinik->get_kategori_izin($id);
		$d['data'] = $this->mod_opklinik->get_opklinik($id);
		$d['Kd_Izin'] = $this->kode_izin; //kode opklinik
		$d['Nm_Izin'] = $this->nama_izin; //kode opklinik
	...
	public function sk($id=0)
	{
		...
		switch($Kd_Kategori_Izin){
			...
			case '2':
				switch($Kd_Jenis_Izin){
					case '1':
					$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk_op',$d);
					break;
					case '2':
					$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk_op_perpanjangan',$d);
					break;
					case '3':
					$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk_op_perubahan',$d);
					break;
					case '4':
					$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk_op_penutupan',$d);
					break;
				}
			break;
		}
	...
	public function sk2($id=0)
	{
		...
		switch($Kd_Kategori_Izin){
			...
			break;
			case '2':
			$this->load->view($this->class_izin.'/'.$this->class_izin.'_cetak_sk2_op',$d);
			break;
		}
	}
	...
	public function get_nomor($id=0,$param=0,$param2=0)
	{
		...
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_opklinik->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_opklinik->get_kategori_izin($id);
		$d['no_sk'] = $this->mod_opklinik->get_nomor_sk($id,$param,$param2); // Kode Param Manual
		$d['id_sk'] = $this->mod_opklinik->get_id_sk($id);;
		$d['Kd_Izin'] = $this->kode_izin; //kode opklinik
		$d['Nm_Izin'] = $this->nama_izin; //kode opklinik
	...
	public function get_agenda($id=0,$param=0)
	{
		...
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_opklinik->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_opklinik->get_kategori_izin($id);
		$d['no_agenda'] = $this->mod_opklinik->get_nomor_agenda($id,2); // Kode Param Manual
		$d['id_agenda'] = $this->mod_opklinik->get_id_agenda($id);;
		$d['Kd_Izin'] = $this->kode_izin; //kode opklinik
		$d['Nm_Izin'] = $this->nama_izin; //kode opklinik
	...
	public function op_perusahaan(){
		//if('IS_AJAX') {
			...
			$d['op_desa'] = $this->mod_opklinik->op_desa();
			$d['op_bentuk_perusahaan'] = $this->mod_opklinik->op_bentuk_perusahaan();
			$d['data'] = $this->mod_opklinik->get_data_perusahaan($idperusahaan);
			$d['data2'] = $this->mod_opklinik->get_data_opklinik($ID_Izin);
			$d['op_status_perusahaan'] = $this->mod_opklinik->op_status_perusahaan();
			$d['op_kategori_opklinik'] = $this->mod_opklinik->op_kategori_opklinik();
			$d['op_kegiatan_usaha'] = $this->mod_opklinik->op_kegiatan_usaha();
			$d['op_kelembagaan'] = $this->mod_opklinik->op_kelembagaan();
	...
	public function laporan()
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(187,$DataAkses);
	...
	public function report($start='',$end='')
	{
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['data']	= $this->mod_opklinik->tbl_print($start,$end);
		$d['tgl_start'] = $start;
		$d['tgl_end'] = $end;
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title);
