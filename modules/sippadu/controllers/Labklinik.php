...
class Labklinik extends CI_Controller {

	var $nama_izin 	= 'Izin Laboratorium Klinik';
	var $class_izin	= 'labklinik';
	var $kode_izin	= 12;
	
	function __construct()
	{
		parent::__construct();
		$this->load->model(array('mod_labklinik','mod_all'));
	...
	public function fo($offset=0)
	{
		...
		$d['PageAkses'] = PageAkses(160,$DataAkses);
		
		if($this->input->post('delete')){
			$this->mod_labklinik->delete_tbl_labklinik();
		}
		...
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_fo(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_fo(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function fo_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_fo(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_fo(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function bo($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(161,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_bo(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_bo(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function bo_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_bo(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_bo(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function kabid($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(162,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('kabid',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_kabid(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_kabid(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function kabid_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('kabid',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_kabid(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_kabid(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function kadin($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(163,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('kadin',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_kadin(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_kadin(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function kadin_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('kadin',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_kadin(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_kadin(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function tu($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(164,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('tu',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_tu(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_tu(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function tu_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('tu',0);
		}
		$d['title'] = $title = $this->nama_izin;
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'tu';
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_tu(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_tu(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function cetak($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(165,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_cetak(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_cetak(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function cetak_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_cetak(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_cetak(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function serah($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(166,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('serah',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_serah(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_serah(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function serah_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_labklinik->verifikasi_tbl_labklinik('serah',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_serah(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_serah(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function serah_win($idizin=0)
	{
		...
		if($this->input->post('submit')){
			$this->mod_labklinik->simpan_pengambil($idizin);
		}
		...
		$d['data'] = $this->mod_labklinik->data_serah($idizin);
	...
	{
		$d['data'] = $this->mod_labklinik->data_serah_ajax();
		$d['Class_Izin'] = $this->class_izin;
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(168,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_labklinik_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_labklinik_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_data($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_labklinik->get_tbl_personal($limit,$offset);
		$d['tot'] = $tot = $this->mod_labklinik->get_tbl_personal()->num_rows();
	...
	public function fo_daftar($jenis_izin=1,$idpelanggan=0,$idperusahaan=0)
	{
		...
		if($this->input->post('submit')){
			$this->mod_labklinik->insert_labklinik_fo();
		}
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Access'] = 'fo';
		$d['Kd_Izin'] = $this->kode_izin; //kode labklinik
		$d['ID_Izin'] = 0;
		$d['op_provinsi'] = $this->mod_labklinik->op_provinsi();
		$d['op_kota'] = $this->mod_labklinik->op_kota();
		$d['op_kecamatan'] = $this->mod_labklinik->op_kecamatan();
		$d['op_desa'] = $this->mod_labklinik->op_desa();
		$d['op_perusahaan'] = $this->mod_labklinik->op_perusahaan($idpelanggan);
		$d['op_bentuk_perusahaan'] = $this->mod_labklinik->op_bentuk_perusahaan();
		$d['data'] = $this->mod_labklinik->get_personal($idpelanggan);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
		$d['op_jenis_izin'] = $this->mod_labklinik->op_jenis_izin();
		$d['op_pengurus_permohonan'] = $this->mod_labklinik->op_pengurus_permohonan();
		$d['op_status_perusahaan'] = $this->mod_labklinik->op_status_perusahaan();
		$d['op_kategori_labklinik'] = $this->mod_labklinik->op_kategori_labklinik();
		$d['op_kegiatan_usaha'] = $this->mod_labklinik->op_kegiatan_usaha();
		$d['op_kelembagaan'] = $this->mod_labklinik->op_kelembagaan();
		$d['op_verifikasi'] = $this->mod_labklinik->op_verifikasi();
		$d['op_serah_terima'] = $this->mod_labklinik->op_serah_terima();
		$d['op_bukti_terima'] = $this->mod_labklinik->op_bukti_terima();
		$d['op_kategori_izin'] = $this->mod_labklinik->op_kategori_izin();
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_labklinik->get_labklinik($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		if($this->input->post('submit')){
			$this->mod_labklinik->update_labklinik($id);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_labklinik->op_bentuk_perusahaan();
		$d['data'] = $this->mod_labklinik->get_personal($idpelanggan);
		$d['data2'] = $this->mod_labklinik->get_data_labklinik($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
		...
		$d['op_bukti_terima'] = $this->mod_labklinik->op_bukti_terima();
		$d['op_kategori_izin'] = $this->mod_labklinik->op_kategori_izin();
		$d['replace'] = $this->mod_labklinik->for_replace($id);
	...
	public function bo_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(160,161),$DataAkses);
	...
	public function fo_proses($id=0,$jenis_izin=0,$idpelanggan=0,$idperusahaan=0)
	{
		...
		if($this->input->post('submit')){
			...
			$this->mod_labklinik->update_labklinik_fo($idperusahaan);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_labklinik->op_bentuk_perusahaan();
		$d['data'] = $this->mod_labklinik->get_personal($idpelanggan);
		$d['data_p'] = $this->mod_labklinik->get_labklinik($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Izin'] = $this->kode_izin; //kode labklinik
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_labklinik->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_labklinik->get_kategori_izin($id);
		$d['data'] = $this->mod_labklinik->get_labklinik($id);
		$d['Kd_Izin'] = $this->kode_izin; //kode labklinik
		$d['Nm_Izin'] = $this->nama_izin; //kode labklinik
	...
	public function get_nomor($id=0,$param=0,$param2=0)
	{
		...
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_labklinik->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_labklinik->get_kategori_izin($id);
		$d['no_sk'] = $this->mod_labklinik->get_nomor_sk($id,$param,$param2); // Kode Param Manual
		$d['id_sk'] = $this->mod_labklinik->get_id_sk($id);;
		$d['Kd_Izin'] = $this->kode_izin; //kode labklinik
		$d['Nm_Izin'] = $this->nama_izin; //kode labklinik
	...
	public function get_agenda($id=0,$param=0)
	{
		...
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_labklinik->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_labklinik->get_kategori_izin($id);
		$d['no_agenda'] = $this->mod_labklinik->get_nomor_agenda($id,2); // Kode Param Manual
		$d['id_agenda'] = $this->mod_labklinik->get_id_agenda($id);;
		$d['Kd_Izin'] = $this->kode_izin; //kode labklinik
		$d['Nm_Izin'] = $this->nama_izin; //kode labklinik
	...
	public function op_perusahaan(){
		//if('IS_AJAX') {
			...
			$d['op_desa'] = $this->mod_labklinik->op_desa();
			$d['op_bentuk_perusahaan'] = $this->mod_labklinik->op_bentuk_perusahaan();
			$d['data'] = $this->mod_labklinik->get_data_perusahaan($idperusahaan);
			$d['data2'] = $this->mod_labklinik->get_data_labklinik($ID_Izin);
			$d['op_status_perusahaan'] = $this->mod_labklinik->op_status_perusahaan();
			$d['op_kategori_labklinik'] = $this->mod_labklinik->op_kategori_labklinik();
			$d['op_kegiatan_usaha'] = $this->mod_labklinik->op_kegiatan_usaha();
			$d['op_kelembagaan'] = $this->mod_labklinik->op_kelembagaan();
	...
	public function laporan()
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(167,$DataAkses);
	...
	public function report($start='',$end='')
	{
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['data']	= $this->mod_labklinik->tbl_print($start,$end);
		$d['tgl_start'] = $start;
		$d['tgl_end'] = $end;
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title);
