...
class Lpks extends CI_Controller {
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(198,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_lpks->get_tbl_lpks_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_lpks->get_tbl_lpks_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_lpks->get_tbl_lpks_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_lpks->get_tbl_lpks_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_lpks->get_lpks($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		$d['op_bentuk_perusahaan'] = $this->mod_lpks->op_bentuk_perusahaan();
		$d['data'] = $this->mod_lpks->get_personal($idpelanggan);
		$d['data2'] = $this->mod_lpks->get_data_lpks($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function bo_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(190,191),$DataAkses);
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(198),$DataAkses);
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_lpks->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_lpks->get_kategori_izin($id);
