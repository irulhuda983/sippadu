...
class Reklame extends CI_Controller {
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(78,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_reklame->get_tbl_reklame_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_reklame->get_tbl_reklame_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_reklame->get_tbl_reklame_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_reklame->get_tbl_reklame_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_reklame->get_reklame($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		if($this->input->post('submit')){
			$this->mod_reklame->update_reklame($id);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_reklame->op_bentuk_perusahaan();
		$d['data'] = $this->mod_reklame->get_personal($idpelanggan);
		$d['data2'] = $this->mod_reklame->get_data_reklame($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(78),$DataAkses);
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_reklame->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_reklame->get_kategori_izin($id);
