...
class Tdup extends CI_Controller {
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(118,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_tdup->get_tbl_tdup_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_tdup->get_tbl_tdup_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_tdup->get_tbl_tdup_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_tdup->get_tbl_tdup_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_daftar($jenis_izin=1,$idpelanggan=0,$idperusahaan=0)
	{
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Access'] = 'fo';
		$d['Kd_Izin'] = $this->kode_izin; //kode tdup
		$d['ID_Izin'] = 0;
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_tdup->get_tdup($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		$d['op_bentuk_perusahaan'] = $this->mod_tdup->op_bentuk_perusahaan();
		$d['data'] = $this->mod_tdup->get_personal($idpelanggan);
		$d['data2'] = $this->mod_tdup->get_data_tdup($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function bo_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(110,111),$DataAkses);
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(118),$DataAkses);
	...
	public function tu_edit($id=0)
	{
		...
		$this->breadcrumb->append_crumb('Register '.$this->nama_izin.'', site_url(fmodule($this->class_izin.'/tu')));
		$this->breadcrumb->append_crumb($title);
		$this->load->view($this->class_izin.'/'.$this->class_izin.'_edit',$d);
	}
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_tdup->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_tdup->get_kategori_izin($id);
		$d['data'] = $this->mod_tdup->get_tdup($id);
		$d['Kd_Izin'] = $this->kode_izin; //kode tdup
		$d['Nm_Izin'] = $this->nama_izin; //kode tdup
