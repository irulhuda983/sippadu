...
class Koperasi extends CI_Controller {
	...
	public function fo($offset=0)
	{
		...
		$d['PageAkses'] = PageAkses(230,$DataAkses);
		
		if($this->input->post('delete')){
			$this->mod_koperasi->delete_tbl_koperasi();
		}
		...
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_fo(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_fo(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function fo_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_fo(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_fo(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function bo($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(231,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_bo(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_bo(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function bo_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_bo(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_bo(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function kabid($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(232,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('kabid',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_kabid(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_kabid(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function kabid_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('kabid',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_kabid(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_kabid(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function kadin($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(233,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('kadin',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_kadin(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_kadin(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function kadin_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('kadin',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_kadin(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_kadin(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function tu($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(234,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('tu',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_tu(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_tu(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function tu_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('tu',0);
		}
		$d['title'] = $title = $this->nama_izin;
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'tu';
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_tu(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_tu(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function cetak($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(235,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_cetak(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_cetak(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function cetak_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_cetak(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_cetak(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function serah($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(236,$DataAkses);
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('serah',1);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_serah(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_serah(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function serah_all($offset=0)
	{
		...
		if($this->input->post('verifikasi')){
			$this->mod_koperasi->verifikasi_tbl_koperasi('serah',0);
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_serah(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_serah(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function serah_win($idizin=0)
	{
		...
		if($this->input->post('submit')){
			$this->mod_koperasi->simpan_pengambil($idizin);
		}
		...
		$d['data'] = $this->mod_koperasi->data_serah($idizin);
	...
	{
		$d['data'] = $this->mod_koperasi->data_serah_ajax();
		$d['Class_Izin'] = $this->class_izin;
	...
	public function rekom($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(238,$DataAkses);
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_rekom(0,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_rekom(0)->num_rows();
		$d['Kd_Verifikasi'] = 1;
	...
	public function rekom_all($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_koperasi_rekom(1,$limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_koperasi_rekom(1)->num_rows();
		$d['Kd_Verifikasi'] = 0;
	...
	public function fo_data($offset=0)
	{
		...
		$d['limit'] = $limit = config('limit');
		$d['offset'] = $offset;
		$d['data'] = $this->mod_koperasi->get_tbl_personal($limit,$offset);
		$d['tot'] = $tot = $this->mod_koperasi->get_tbl_personal()->num_rows();
	...
	public function fo_daftar($jenis_izin=1,$idpelanggan=0,$idperusahaan=0)
	{
		...
		if($this->input->post('submit')){
			$this->mod_koperasi->insert_koperasi_fo();
		}
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Access'] = 'fo';
		$d['Kd_Izin'] = $this->kode_izin; //kode koperasi
		$d['ID_Izin'] = 0;
		$d['op_provinsi'] = $this->mod_koperasi->op_provinsi();
		$d['op_kota'] = $this->mod_koperasi->op_kota();
		$d['op_kecamatan'] = $this->mod_koperasi->op_kecamatan();
		$d['op_desa'] = $this->mod_koperasi->op_desa();
		$d['op_perusahaan'] = $this->mod_koperasi->op_perusahaan($idpelanggan);
		$d['op_bentuk_perusahaan'] = $this->mod_koperasi->op_bentuk_perusahaan();
		$d['data'] = $this->mod_koperasi->get_personal($idpelanggan);
		...
		$d['op_status_perusahaan'] = $this->mod_koperasi->op_status_perusahaan();
		$d['op_kategori_koperasi'] = $this->mod_koperasi->op_kategori_koperasi();
		$d['op_kegiatan_usaha'] = $this->mod_koperasi->op_kegiatan_usaha();
		$d['op_kelembagaan'] = $this->mod_koperasi->op_kelembagaan();
		$d['op_verifikasi'] = $this->mod_koperasi->op_verifikasi();
		$d['op_serah_terima'] = $this->mod_koperasi->op_serah_terima();
		$d['op_bukti_terima'] = $this->mod_koperasi->op_bukti_terima();
		$d['op_kategori_izin'] = $this->mod_koperasi->op_kategori_izin();
	...
	public function fo_edit($id=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_koperasi->get_koperasi($id);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		if($this->input->post('submit')){
			$this->mod_koperasi->update_koperasi($id);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_koperasi->op_bentuk_perusahaan();
		$d['data'] = $this->mod_koperasi->get_personal($idpelanggan);
		$d['data2'] = $this->mod_koperasi->get_data_koperasi($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
		...
		$d['op_bukti_terima'] = $this->mod_koperasi->op_bukti_terima();
		$d['op_kategori_izin'] = $this->mod_koperasi->op_kategori_izin();
		$d['replace'] = $this->mod_koperasi->for_replace($id);
	...
	public function bo_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(230,231),$DataAkses);
	...
	public function rekom_edit($id=0)
	{
		...
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(array(238),$DataAkses);
	...
	public function fo_proses($id=0,$jenis_izin=0,$idpelanggan=0,$idperusahaan=0)
	{
		...
		if($this->input->post('submit')){
			...
			$this->mod_koperasi->update_koperasi_fo($idperusahaan);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_koperasi->op_bentuk_perusahaan();
		$d['data'] = $this->mod_koperasi->get_personal($idpelanggan);
		$d['data_p'] = $this->mod_koperasi->get_koperasi($id);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Izin'] = $this->kode_izin; //kode koperasi
		$d['Kd_Jenis'] = $jenis_izin;
	...
	public function tt($id=0)
	{
		...
		$d['menuactive'] = $this->class_izin.'_cetak';
		$d['Class_Izin'] = $this->class_izin;
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_koperasi->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_koperasi->get_kategori_izin($id);
		$d['data'] = $this->mod_koperasi->get_koperasi($id);
		$d['Kd_Izin'] = $this->kode_izin; //kode koperasi
		$d['Nm_Izin'] = $this->nama_izin; //kode koperasi
	...
	public function get_nomor($id=0,$param=0,$param2=0)
	{
		...
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_koperasi->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_koperasi->get_kategori_izin($id);
		$d['no_sk'] = $this->mod_koperasi->get_nomor_sk($id,$param,$param2); // Kode Param Manual
		$d['id_sk'] = $this->mod_koperasi->get_id_sk($id);;
		$d['Kd_Izin'] = $this->kode_izin; //kode koperasi
		$d['Nm_Izin'] = $this->nama_izin; //kode koperasi
	...
	public function get_agenda($id=0,$param=0)
	{
		...
		$d['Kd_Jenis_Izin'] = $Kd_Jenis_Izin = $this->mod_koperasi->get_jenis_izin($id);
		$d['Kd_Kategori_Izin'] = $Kd_Kategori_Izin = $this->mod_koperasi->get_kategori_izin($id);
		$d['no_agenda'] = $this->mod_koperasi->get_nomor_agenda($id,2); // Kode Param Manual
		$d['id_agenda'] = $this->mod_koperasi->get_id_agenda($id);;
		$d['Kd_Izin'] = $this->kode_izin; //kode koperasi
		$d['Nm_Izin'] = $this->nama_izin; //kode koperasi
	...
	public function op_perusahaan(){
		//if('IS_AJAX') {
			...
			$d['op_desa'] = $this->mod_koperasi->op_desa();
			$d['op_bentuk_perusahaan'] = $this->mod_koperasi->op_bentuk_perusahaan();
			$d['data'] = $this->mod_koperasi->get_data_perusahaan($idperusahaan);
			$d['data2'] = $this->mod_koperasi->get_data_koperasi($ID_Izin);
			$d['op_status_perusahaan'] = $this->mod_koperasi->op_status_perusahaan();
			$d['op_kategori_koperasi'] = $this->mod_koperasi->op_kategori_koperasi();
			$d['op_kegiatan_usaha'] = $this->mod_koperasi->op_kegiatan_usaha();
			$d['op_kelembagaan'] = $this->mod_koperasi->op_kelembagaan();
	...
	public function laporan()
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(237,$DataAkses);
	...
	public function report($start='',$end='')
	{
		...
		$d['Class_Izin'] = $this->class_izin;
		$d['data']	= $this->mod_koperasi->tbl_print($start,$end);
		$d['tgl_start'] = $start;
		$d['tgl_end'] = $end;
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title);
