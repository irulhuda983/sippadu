...
class Setting extends CI_Controller {
	...
	public function mod_izin_add()
	{
		...
		$d['menuopen2'] = 'setting';
		$d['menuactive'] = 'setting_mod_izin';
		$d['op_skpd'] = $this->mod_setting->op_skpd();
	...
	public function skpd_rekom ($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(36,$DataAkses);
		
		if($this->input->post('delete')){
			$a = $this->mod_setting->delete_skpd();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_setting->enable_skpd();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_setting->disable_skpd();
		}
		
		$d['title'] = $title = 'SKPD Pembuat Rekomendasi';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'setting';
		$d['menuactive'] = 'setting_skpd_rekom';
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $this->mod_setting->skpd_rekom($limit,$offset);
		$d['tot'] = $tot = $this->mod_setting->skpd_rekom()->num_rows();
		$config['base_url'] = $url_parent = site_url(fmodule('setting/skpd_rekom'));
		$config['total_rows'] = $tot;
		$config['per_page'] = $limit;
		$config['uri_segment'] = 3;
		$config['first_link'] = lang('First');
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('setting/setting_skpd_rekom',$d);
	}
	
	public function skpd_rekom_add()
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(36,$DataAkses);
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_setting->insert_skpd_rekom($logo,$ttd,$stempel);
		}
		
		$d['title'] = $title = 'Tambah SKPD Pembuat Rekomendasi';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'setting';
		$d['menuopen2'] = 'setting';
		$d['menuactive'] = 'setting_skpd_rekom';
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('SKPD Pembuat Rekomendasi', site_url(fmodule('setting/skpd_rekom')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('setting/setting_skpd_rekom_add',$d);
		
	}
	
	public function skpd_rekom_edit($id=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(36,$DataAkses);
		...
		if($this->input->post('submit')){
			...
			if ($this->input->post('hapus_logo') || $_FILES['Logo']['name']){
				$dataimg= $this->mod_setting->data_skpd_rekom($id);
				$path = './assets/'.config('theme').'/images/setting/';
			...
			if ($this->input->post('hapus_ttd') || $_FILES['TTD']['name']){
				$dataimg2= $this->mod_setting->data_skpd_rekom($id);
				$path2 = './assets/'.config('theme').'/images/setting/';
				if($dataimg2->num_rows() > 0){
					$di2 = $dataimg2->row();
					$f_asli2 = $di2->TTD;
					$img_asli2 = $path2.$f_asli2;
					file_exists($img_asli2) ? unlink($img_asli2) : '' ;
				}
				$ttd	= ' '; 
			}
			
			if ($this->input->post('hapus_stempel') || $_FILES['Stempel']['name']){
				$dataimg3= $this->mod_setting->data_skpd_rekom($id);
				$path3 = './assets/'.config('theme').'/images/setting/';
				if($dataimg3->num_rows() > 0){
					$di3 = $dataimg3->row();
					$f_asli3 = $di3->Stempel;
					$img_asli3 = $path3.$f_asli3;
					file_exists($img_asli3) ? unlink($img_asli3) : '' ;
				}
				$stempel	= ' '; 
			}
			...
			$a = $this->mod_setting->update_skpd_rekom($id,$logo,$ttd,$stempel);
		}
		
		$d['title'] = $title = 'Edit SKPD Pembuat Rekomendasi';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'setting';
		$d['menuopen2'] = 'setting';
		$d['menuactive'] = 'setting_skpd_rekom';
		$d['data'] = $this->mod_setting->data_skpd_rekom($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('SKPD Pembuat Rekomendasi', site_url(fmodule('setting/skpd_rekom')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('setting/setting_skpd_rekom_edit',$d);
