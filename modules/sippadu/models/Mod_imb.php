...
class Mod_imb extends CI_Model {
	
	function get_tbl_imb_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (tbimb.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR tbimb.Nm_Usaha like "%'.$keyword.'%" OR tbimb.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		FROM
		tbimb
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbimb.ID_Pelanggan WHERE tbimb.Status = 1 AND tbimb.Flow_FO = 1 and tbimb.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_imb_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_imb_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbimb
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbimb.ID_Pelanggan WHERE tbimb.Status = 1 AND tbimb.Flow_FO = 1 and tbimb.Flow_BO = 1 and tbimb.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_imb_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbimb
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbimb.ID_Pelanggan WHERE tbimb.Status = 1 AND tbimb.Flow_FO = 1 and tbimb.Flow_BO = 1 and tbimb.Flow_Kabid = 1 and tbimb.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_imb_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbimb
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbimb.ID_Pelanggan WHERE tbimb.Status = 1 AND tbimb.Flow_FO = 1 and tbimb.Flow_BO = 1 and tbimb.Flow_Kabid = 1 and tbimb.Flow_Kadin = 1 and tbimb.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_imb_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbimb
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbimb.ID_Pelanggan WHERE tbimb.Status = 1 AND tbimb.Flow_FO = 1 and tbimb.Flow_BO = 1 and tbimb.Flow_Kabid = 1 and tbimb.Flow_Kadin = 1 and tbimb.Flow_TU = 1 and tbimb.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_imb_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_imb_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbimb
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbimb.ID_Pelanggan WHERE tbimb.Status = 1 AND tbimb.Flow_FO = 1 and tbimb.Flow_BO = 0 and tbimb.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_imb($id=0){
		$a = $this->db->where('ID',$id)->get('tbimb');
		return $a;
	}
	
	function delete_tbl_imb(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('tbimb',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('tbimb',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_imb($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
	...
	function insert_imb_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('tbimb');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbimb');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/IMB/'.TimeNow('Y');
	...
	function update_imb($id=0){
		...
		if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
			$this->form_validation->set_rules('op_kategori_imb','Jenis Bangunan','is_natural_no_zero');
			$this->form_validation->set_rules('op_peruntukan_imb','Peruntukan Bangunan','is_natural_no_zero');
			...
			$this->form_validation->set_rules('kepemilikan','Kepemilikan Tanah','');
			$this->form_validation->set_rules('no_shm','No SHM','');
		...
		if($Kd_Access == 'tu'){
			...
			//$this->form_validation->set_rules('tgl_ska','Tanggal SK Akhir','required');
		}
		
		
		if($this->form_validation->run() == TRUE){
			
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbimb');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			$wh2['ID'] = $id;
			//$in2['IDIMB'] = str_pad($last_id,5,0,STR_PAD_LEFT).'/IMB/'.TimeNow('Y');
			$in2['No_Surat_Mohon'] = $this->input->post('no_mohon');
			$in2['Tgl_Mohon'] = db_time($this->input->post('tgl_mohon'));
			$in2['ID_Pelanggan'] = $this->input->post('idpelanggan');
			$in2['Kd_Pengurus'] = $this->input->post('op_pengurus_permohonan');
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				$in2['Jenis_Bangunan'] = $this->input->post('op_kategori_imb');
				$in2['Peruntukan_Bangunan'] = $this->input->post('op_peruntukan_imb');
				...
				$in2['No_SHM'] = $this->input->post('no_shm');
			...
			$a = $this->db->update('tbimb',$in2,$wh2);
	...
	function op_jenis_izin(){
		...
		$h['1'] = 'Baru';
		$h['2'] = 'Balik Nama';
		$h['3'] = 'Balik Nama dan Tambahan Bangunan';
		return $h;
	}
	...
	function op_kategori_imb(){
		$h = array();
		$h['0'] = '- Pilih Kategori -';
		$h['1'] = 'Konstruksi Bangunan Permanen';
		$h['2'] = 'Pemasangan Reklame';
		return $h;
	}
	
	function op_peruntukan_imb(){
		$h = array();
		$h['0'] = '- Pilih Kategori -';
		$h['1'] = 'Kegiatan Usaha';
		$h['2'] = 'Kegiatan Non Usaha';
		return $h;
	}
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('tbimb');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbimb');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbimb');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbimb');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('tbimb');
			$dd = $d->first_row();
	...
	function for_replace($id=0){
		...
		$h = array(
				...
				'[NOSHM]'				=> $aa->No_SHM,
				'[NAMASHM]'				=> $aa->Nm_SHM,
				'[ALAMATUSAHA]'			=> $aa->Lokasi.' '.desa($aa->Kd_Desa).', '.kecamatan($aa->Kd_Kecamatan).', '.kota($aa->Kd_Kota).', '.provinsi($aa->Kd_Provinsi),
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM tbimb WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM tbimb WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM tbimb a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
