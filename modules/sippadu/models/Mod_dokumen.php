...
class Mod_dokumen extends CI_Model {
	...
	function op_izin(){
		$a = $this->db->where('Status',1)->order_by('Kd_Izin','ASC')->get('tb_izin');
		$h = array();
		$sess = $this->session->userdata('userid');
		if($a->num_rows() > 0){
			foreach($a->result() as $aa){
				$h['no'] = '- Pilih Izin -';
				$h[$aa->Kd_Izin] = ($sess==1? '['.$aa->Kd_Izin.'] -- ' : '').$aa->Nm_Izin;
			}
		}
		else{
			$h['no'] = '- Pilih Izin -';
	...
	function op_jenis_parameter($Kd_Izin=0){
		...
		if($a->num_rows() > 0){
			foreach($a->result() as $aa){
				...
				$h[$aa->Param] = ($sess==1? '['.$aa->Param.'] -- ' : '').$aa->Keterangan;
			}
		}
		else{
			$h['no'] = '- Pilih Parameter -';
	...
	function save_master_dok($idizin=0,$idjenis=0){
		if($idizin > 0 and $idjenis > 0){
			...
			set_alert('error','Pilih opsi Setting dengan benar');
			redirect(current_url());
		}
	}
	...
	function get_list_parameter($idizin=0){
		$a = $this->db->where('Kd_Izin',$idizin)->get('tb_izin');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->row();
			$h = $aa->Parameter;
	...
	function insert_parameter($idizin=0,$idjenis=0){
		$this->form_validation->set_rules('param','Parameter','required');
		$this->form_validation->set_rules('format','Format','');
		if($this->form_validation->run() == TRUE){
			$in['Kd_Izin'] = $idizin;
			$in['Param'] = $this->input->post('param');
			$in['Type_Format'] = $this->input->post('type');
			//$in['Format'] = $this->input->post('format');
			$in['Keterangan'] = $this->input->post('nm_param');
			$in['Status'] = 1;
			$a = $this->db->insert('tb_parameter',$in);
			if($a){
				set_alert('success','Data berhasil disimpan');
				redirect(current_url());
			}
	...
	function update_parameter($idizin=0,$idjenis=0){
		$this->form_validation->set_rules('param','Parameter','required');
		$this->form_validation->set_rules('format','Format','');
		if($this->form_validation->run() == TRUE){
			$wh['Kd_Izin'] = $idizin;
	...
	function get_last_parameter($idizin=0){
		$qlast = $this->db->select_max('Param','Param')->where('Kd_Izin',$idizin)->get('tb_parameter');
		$last_id = 1;
		if($qlast->num_rows() > 0){
			$ls = $qlast->row();
			$last_id = $ls->Param+1;
	...
	function delete_master_parameter($idizin=0,$idjenis=0){
		$wh['Kd_Izin'] = $idizin;
		$wh['Param'] = $idjenis;
		$a = $this->db->delete('tb_parameter',$wh);
		if($a){
			set_alert('success','Data berhasil dihapus');
			redirect(current_url());
		}
		else{
			set_alert('error','Data gagal dihapus');
			redirect(current_url());
		}
	}
