...
class Mod_opklinik extends CI_Model {
	
	function get_tbl_opklinik_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (sippadu_opklinik.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR sippadu_opklinik.Nm_Usaha like "%'.$keyword.'%" OR sippadu_opklinik.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		sippadu_opklinik.ID,
		sippadu_opklinik.ID_Reg,
		sippadu_opklinik.ID_Pelanggan,
		sippadu_opklinik.ID_Usaha,
		"" AS Kd_Jns,
		sippadu_opklinik.Jenis_Izin,
		sippadu_opklinik.Nm_Usaha,
		sippadu_opklinik.No_SK,
		sippadu_opklinik.Tgl_SK,
		sippadu_opklinik.Flow_FO,
		sippadu_opklinik.Flow_BO,
		sippadu_opklinik.Flow_Kabid,
		sippadu_opklinik.Flow_Kadin,
		sippadu_opklinik.Flow_TU,
		sippadu_opklinik.Flow_Serah,
		sippadu_opklinik.Time_Buat,
		sippadu_opklinik.Time_Selesai,
		...
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan WHERE sippadu_opklinik.Status = 1 AND sippadu_opklinik.Flow_FO = 1 and sippadu_opklinik.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_opklinik_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_opklinik_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan WHERE sippadu_opklinik.Status = 1 AND sippadu_opklinik.Flow_FO = 1 and sippadu_opklinik.Flow_BO = 1 and sippadu_opklinik.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_opklinik_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan WHERE sippadu_opklinik.Status = 1 AND sippadu_opklinik.Flow_FO = 1 and sippadu_opklinik.Flow_BO = 1 and sippadu_opklinik.Flow_Kabid = 1 and sippadu_opklinik.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_opklinik_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan WHERE sippadu_opklinik.Status = 1 AND sippadu_opklinik.Flow_FO = 1 and sippadu_opklinik.Flow_BO = 1 and sippadu_opklinik.Flow_Kabid = 1 and sippadu_opklinik.Flow_Kadin = 1 and sippadu_opklinik.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_opklinik_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan WHERE sippadu_opklinik.Status = 1 AND sippadu_opklinik.Flow_FO = 1 and sippadu_opklinik.Flow_BO = 1 and sippadu_opklinik.Flow_Kabid = 1 and sippadu_opklinik.Flow_Kadin = 1 and sippadu_opklinik.Flow_TU = 1 and sippadu_opklinik.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_opklinik_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		sippadu_opklinik.Time_Buat,
		sippadu_opklinik.Time_Selesai,
		sippadu_opklinik.Nm_Pengambil,
		sippadu_opklinik.Time_Diambil,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		tbpelanggan.PROVINSI,
		tbpelanggan.KOTA,
	...
	function get_tbl_opklinik_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan WHERE sippadu_opklinik.Status = 1 AND sippadu_opklinik.Flow_FO = 1 and sippadu_opklinik.Flow_BO = 0 and sippadu_opklinik.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_opklinik($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_opklinik');
		return $a;
	}
	
	function delete_tbl_opklinik(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('sippadu_opklinik',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('sippadu_opklinik',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_opklinik($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('sippadu_opklinik',$in,$wh);
			}
	...
	function insert_opklinik_fo(){
		...
		$this->form_validation->set_rules('pemilik','Pemilik','');
		$this->form_validation->set_rules('penanggungjawab','Penanggungjawab','');
		$this->form_validation->set_rules('jenis_klinik','Jenis Klinik','');
		$this->form_validation->set_rules('jenis_pelayanan','Jenis Pelayanan','');
		$this->form_validation->set_rules('akta_pendirian','Nomor Akta Pendirian','');
		$this->form_validation->set_rules('akta_tanggal','Tanggal Akta Pendirian','');
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('sippadu_opklinik');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_opklinik');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/OPKLINIK/'.TimeNow('Y');
			...
			$in2['Kategori'] = $this->input->post('kategori_izin');
			$in2['Pemilik'] = $this->input->post('pemilik');
			$in2['Penanggungjawab'] = $this->input->post('penanggungjawab');
			$in2['Jenis_Klinik'] = $this->input->post('jenis_klinik');
			$in2['Jenis_Pelayanan'] = $this->input->post('jenis_pelayanan');
			$in2['Akta_Pendirian'] = $this->input->post('akta_pendirian');
			$in2['Akta_Tanggal'] = db_time($this->input->post('akta_tanggal'));
			$in2['Tembusan'] = $this->input->post('tembusan');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('sippadu_opklinik',$in2);
	...
	function update_opklinik($id=0){
		...
		if($Kd_Access == 'bo'){
			...
			$this->form_validation->set_rules('judul','Judul Izin',''); 
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_opklinik');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['Judul_Izin'] = $this->input->post('judul');
			...
			$a = $this->db->update('sippadu_opklinik',$in2,$wh2);
	...
	function get_data_opklinik($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_opklinik');
		return $a;
	}
	...
	function op_kategori_izin(){
		$h = array();
		$h['0'] = '-';
		$h['1'] = 'IZIN PENDIRIAN KLINIK';
		$h['2'] = 'IZIN OPERASIONAL KLINIK';
		return $h;
	}
	
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		
		$a = $this->db->where('ID',$idizin)->get('sippadu_opklinik');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_opklinik');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_opklinik');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('sippadu_opklinik');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('sippadu_opklinik');
			$dd = $d->first_row();
	...
	function tbl_print($start='',$end=''){
		...
		$a = $this->db->query('SELECT
		sippadu_opklinik.ID_Reg AS ID_IZIN,
		sippadu_opklinik.Time_Buat AS TGL_BUAT,
		"Izin Tenaga Kesehatan" AS NM_IZIN,
		"" AS KODEJENIS,
		right(sippadu_opklinik.Jenis_Izin,1) AS JENIS_IZIN,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		...
		sippadu_opklinik.Nm_Usaha AS NM_PERUSAHAAN,
		tbpelanggan.NO_HP,
		0 AS INVESTASI,
		0 AS NAKER_L,
		0 AS NAKER_P,
		sippadu_opklinik.Nm_FO,
		sippadu_opklinik.Nm_BO,
		sippadu_opklinik.Nm_Pengambil,
		sippadu_opklinik.Nm_Pengurus AS NM_PENGURUS,
		sippadu_opklinik.Time_Selesai AS TGL_SELESAI,
		sippadu_opklinik.Time_Diambil AS TGL_DIAMBIL
		FROM
		sippadu_opklinik
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_opklinik.ID_Pelanggan
		/* INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = sippadu_opklinik.ID_Usaha */
	...
	function for_replace($id=0){
		...
		$h = array(
				...
				'[NAMAKLINIK]'			=> $aa->Nm_Usaha,
				'[ALAMATKLINIK]'			=> ($aa->Alamat_Usaha != '' ? $aa->Alamat_Usaha.' - '.desa($aa->Kd_Desa).'<br/>'.kecamatan($aa->Kd_Kecamatan).' '.kota($aa->Kd_Kota) : desa($aa->Kd_Desa).' '.kecamatan($aa->Kd_Kecamatan).' '.kota($aa->Kd_Kota)),
				'[PERIHAL]'				=> $aa->Perihal,
				'[ALAMATUSAHA]'			=> $aa->Lokasi,
				'[PEMILIK]'			=> $aa->Pemilik,
				'[PENANGGUNGJAWAB]'			=> $aa->Penanggungjawab,
				'[NOSURATREKOM]'		=> $aa->No_Surat_Rekom,
				'[TGLSURATREKOM]'		=> Date_Indo(TimeFormat($aa->Tgl_Surat_Rekom,'d F Y')),
				'[JENISKLINIK]'		=> $aa->Jenis_Klinik,
				'[JENISPELAYANAN]'		=> $aa->Jenis_Pelayanan,
				'[AKTAPENDIRIAN]'		=> $aa->Akta_Pendirian,
				'[TGLAKTAPENDIRIAN]'		=> Date_Indo(TimeFormat($aa->Akta_Tanggal,'d F Y')),
				'[TGLMOHON]'		=> Date_Indo(TimeFormat($aa->Tgl_Mohon,'d F Y')),
				'[TGLSKAWAL]'		=> Date_Indo(TimeFormat($aa->Tgl_SK,'d F Y')),
				'[TGLSKAKHIR]'		=> Date_Indo(TimeFormat($aa->Tgl_SKA,'d F Y')),
			);
		return $h;
	}
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM sippadu_opklinik WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM sippadu_opklinik WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM sippadu_opklinik a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('sippadu_opklinik',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
