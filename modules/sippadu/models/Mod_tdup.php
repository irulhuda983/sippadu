...
class Mod_tdup extends CI_Model {
	
	function get_tbl_tdup_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (tbtdup.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR tbtdup.Nm_Usaha like "%'.$keyword.'%" OR tbtdup.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		FROM
		tbtdup
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdup.ID_Pelanggan WHERE tbtdup.Status = 1 AND tbtdup.Flow_FO = 1 and tbtdup.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdup_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_tdup_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdup
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdup.ID_Pelanggan WHERE tbtdup.Status = 1 AND tbtdup.Flow_FO = 1 and tbtdup.Flow_BO = 1 and tbtdup.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdup_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdup
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdup.ID_Pelanggan WHERE tbtdup.Status = 1 AND tbtdup.Flow_FO = 1 and tbtdup.Flow_BO = 1 and tbtdup.Flow_Kabid = 1 and tbtdup.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdup_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdup
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdup.ID_Pelanggan WHERE tbtdup.Status = 1 AND tbtdup.Flow_FO = 1 and tbtdup.Flow_BO = 1 and tbtdup.Flow_Kabid = 1 and tbtdup.Flow_Kadin = 1 and tbtdup.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdup_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdup
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdup.ID_Pelanggan WHERE tbtdup.Status = 1 AND tbtdup.Flow_FO = 1 and tbtdup.Flow_BO = 1 and tbtdup.Flow_Kabid = 1 and tbtdup.Flow_Kadin = 1 and tbtdup.Flow_TU = 1 and tbtdup.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdup_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_tdup_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdup
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdup.ID_Pelanggan WHERE tbtdup.Status = 1 AND tbtdup.Flow_FO = 1 and tbtdup.Flow_BO = 0 and tbtdup.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tdup($id=0){
		$a = $this->db->where('ID',$id)->get('tbtdup');
		return $a;
	}
	
	function delete_tbl_tdup(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('tbtdup',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('tbtdup',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_tdup($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
	...
	function insert_tdup_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('tbtdup');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdup');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/TDUP/'.TimeNow('Y');
	...
	function update_tdup($id=0){
		...
		if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
			...
			$this->form_validation->set_rules('no_akte','No Akte','');
			$this->form_validation->set_rules('tgl_akte','Tanggal Akte','');
			$this->form_validation->set_rules('nama_izin_teknis','Nama Ijin Teknis','');
			$this->form_validation->set_rules('no_izin_teknis','No Izin Teknis','');
			$this->form_validation->set_rules('no_ho','No HO','');
			$this->form_validation->set_rules('tgl_ho','Tanggal HO','');
			$this->form_validation->set_rules('no_imb','No IMB','');
			$this->form_validation->set_rules('tgl_imb','Tgl IMB','');
			...
			$this->form_validation->set_rules('sk_tembusan','SK Tembusan','');
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdup');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			$wh2['ID'] = $id;
			//$in2['IDTDUP'] = str_pad($last_id,5,0,STR_PAD_LEFT).'/TDUP/'.TimeNow('Y');
			$in2['No_Surat_Mohon'] = $this->input->post('no_mohon');
			$in2['Tgl_Mohon'] = db_time($this->input->post('tgl_mohon'));
			$in2['ID_Pelanggan'] = $this->input->post('idpelanggan');
			$in2['Kd_Pengurus'] = $this->input->post('op_pengurus_permohonan');
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['No_Akte'] = $this->input->post('no_akte');
				$in2['Tgl_Akte'] = db_time($this->input->post('tgl_akte'));
				$in2['Nm_Izin_Teknis'] = $this->input->post('nama_izin_teknis');
				$in2['No_Izin_Teknis'] = $this->input->post('no_izin_teknis');
				$in2['No_HO'] = $this->input->post('no_ho');
				$in2['Tgl_HO'] = db_time($this->input->post('tgl_ho'));
				$in2['No_IMB'] = $this->input->post('no_imb');
				$in2['Tgl_IMB'] = db_time($this->input->post('tgl_imb'));
				$in2['Tembusan'] = $this->input->post('sk_tembusan');
			...
			$a = $this->db->update('tbtdup',$in2,$wh2);
	...
	function op_kategori_tdup(){
		$h = array();
		$h['0'] = '- Pilih Kategori TDUP -';
		$h['1'] = 'TDUP Mikro';
		$h['2'] = 'TDUP Kecil';
		$h['3'] = 'TDUP Menengah';
		$h['4'] = 'TDUP Besar';
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('tbtdup');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdup');
		$h = 0;
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdup');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbtdup');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('tbtdup');
			$dd = $d->first_row();
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM tbtdup WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM tbtdup WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM tbtdup a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
