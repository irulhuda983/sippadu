...
class Mod_apotek extends CI_Model {
	
	function get_tbl_apotek_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (sippadu_apotek.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR sippadu_apotek.Nm_Usaha like "%'.$keyword.'%" OR sippadu_apotek.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		sippadu_apotek.ID,
		sippadu_apotek.ID_Reg,
		sippadu_apotek.ID_Pelanggan,
		sippadu_apotek.ID_Usaha,
		"" AS Kd_Jns,
		sippadu_apotek.Jenis_Izin,
		sippadu_apotek.Nm_Usaha,
		sippadu_apotek.No_SK,
		sippadu_apotek.Tgl_SK,
		sippadu_apotek.Flow_FO,
		sippadu_apotek.Flow_BO,
		sippadu_apotek.Flow_Kabid,
		sippadu_apotek.Flow_Kadin,
		sippadu_apotek.Flow_TU,
		sippadu_apotek.Flow_Serah,
		sippadu_apotek.Time_Buat,
		sippadu_apotek.Time_Selesai,
		...
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan WHERE sippadu_apotek.Status = 1 AND sippadu_apotek.Flow_FO = 1 and sippadu_apotek.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_apotek_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_apotek_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan WHERE sippadu_apotek.Status = 1 AND sippadu_apotek.Flow_FO = 1 and sippadu_apotek.Flow_BO = 1 and sippadu_apotek.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_apotek_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan WHERE sippadu_apotek.Status = 1 AND sippadu_apotek.Flow_FO = 1 and sippadu_apotek.Flow_BO = 1 and sippadu_apotek.Flow_Kabid = 1 and sippadu_apotek.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_apotek_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan WHERE sippadu_apotek.Status = 1 AND sippadu_apotek.Flow_FO = 1 and sippadu_apotek.Flow_BO = 1 and sippadu_apotek.Flow_Kabid = 1 and sippadu_apotek.Flow_Kadin = 1 and sippadu_apotek.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_apotek_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan WHERE sippadu_apotek.Status = 1 AND sippadu_apotek.Flow_FO = 1 and sippadu_apotek.Flow_BO = 1 and sippadu_apotek.Flow_Kabid = 1 and sippadu_apotek.Flow_Kadin = 1 and sippadu_apotek.Flow_TU = 1 and sippadu_apotek.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_apotek_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		sippadu_apotek.Time_Buat,
		sippadu_apotek.Time_Selesai,
		sippadu_apotek.Nm_Pengambil,
		sippadu_apotek.Time_Diambil,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		tbpelanggan.PROVINSI,
		tbpelanggan.KOTA,
	...
	function get_tbl_apotek_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan WHERE sippadu_apotek.Status = 1 AND sippadu_apotek.Flow_FO = 1 and sippadu_apotek.Flow_BO = 0 and sippadu_apotek.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_apotek($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_apotek');
		return $a;
	}
	
	function delete_tbl_apotek(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('sippadu_apotek',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('sippadu_apotek',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_apotek($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('sippadu_apotek',$in,$wh);
			}
	...
	function insert_apotek_fo(){
		...
		$this->form_validation->set_rules('sebab','Sebab Penutupan','');
		
		$this->form_validation->set_rules('nama_usaha','Nama Apotek','');
		$this->form_validation->set_rules('jabatan_pemohon','Jabatan Pemohon','');
		$this->form_validation->set_rules('nama_pemilik','Nama Pemilik Apotek','');
		$this->form_validation->set_rules('nama_apoteker','Nama Apoteker','');
		$this->form_validation->set_rules('alamat_usaha','Alamat Apotek','');
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('sippadu_apotek');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_apotek');
			$last_reg = 1;
			if($q_last_reg->num_rows() > 0){
				$a_last_reg = $q_last_reg->row();
				$last_reg = $a_last_reg->No_Reg + 1;
			}
			$idReg = str_pad($last_reg,5,0,STR_PAD_LEFT).'/APOTEK/'.TimeNow('Y');
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = $idReg;
			...
			$in2['Sebab_Penutupan'] = $this->input->post('sebab');
			...
			$in2['Nm_Apoteker'] = $this->input->post('nama_apoteker');
			$in2['Pemilik_Usaha'] = $this->input->post('nama_pemilik');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('sippadu_apotek',$in2);
			...
			if($a){
				$track = [
					...
					'pesan' => 'Verifikasi Dokumen oleh Front Office'
				];
	...
	function update_apotek($id=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_apotek');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			$a = $this->db->update('sippadu_apotek',$in2,$wh2);
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('sippadu_apotek');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_apotek');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_apotek');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('sippadu_apotek');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('sippadu_apotek');
			$dd = $d->first_row();
	...
	function tbl_print($start='',$end=''){
		...
		$a = $this->db->query('SELECT
		sippadu_apotek.ID_Reg AS ID_IZIN,
		sippadu_apotek.Time_Buat AS TGL_BUAT,
		"Izin Tenaga Kesehatan" AS NM_IZIN,
		"" AS KODEJENIS,
		right(sippadu_apotek.Jenis_Izin,1) AS JENIS_IZIN,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		...
		sippadu_apotek.Nm_Usaha AS NM_PERUSAHAAN,
		tbpelanggan.NO_HP,
		0 AS INVESTASI,
		0 AS NAKER_L,
		0 AS NAKER_P,
		sippadu_apotek.Nm_FO,
		sippadu_apotek.Nm_BO,
		sippadu_apotek.Nm_Pengambil,
		sippadu_apotek.Nm_Pengurus AS NM_PENGURUS,
		sippadu_apotek.Time_Selesai AS TGL_SELESAI,
		sippadu_apotek.Time_Diambil AS TGL_DIAMBIL
		FROM
		sippadu_apotek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_apotek.ID_Pelanggan
		/* INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = sippadu_apotek.ID_Usaha */
	...
	function for_replace($id=0){
		...
		$h = array(
				'[TAHUN]' 		=> TimeNow('Y'),
				'[NAMAPEMOHON]' 		=> pelanggan($idpel,'nama'),
				...
				'[PERIHAL]'				=> $aa->Perihal,
				//'[NAMASHM]'				=> $aa->Nm_SHM,
				'[NAMAAPOTEKER]'			=> $aa->Nm_Apoteker,
				'[NAMAAPOTEK]'			=> $aa->Nm_Usaha,
				'[ALAMATAPOTEK]'			=> ($aa->Alamat_Usaha != '' ? $aa->Alamat_Usaha.' - '.desa($aa->Kd_Desa).'<br/>'.kecamatan($aa->Kd_Kecamatan).' '.kota($aa->Kd_Kota) : desa($aa->Kd_Desa).' '.kecamatan($aa->Kd_Kecamatan).' '.kota($aa->Kd_Kota)),
				'[PSA]'					=> $aa->Pemilik_Usaha,
				'[JABATANPEMOHON]'		=> $aa->Jabatan_Pemohon,
				'[NOSURATREKOM]'		=> $aa->No_Surat_Rekom,
				'[TGLSURATREKOM]'		=> Date_Indo(TimeFormat($aa->Tgl_Surat_Rekom,'d F Y')),
				'[SEBABPENUTUPAN]'		=> $aa->Sebab_Penutupan,
			);
		return $h;
	}
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM sippadu_apotek WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM sippadu_apotek WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM sippadu_apotek a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('sippadu_apotek',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
