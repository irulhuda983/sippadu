...
class Mod_trayek extends CI_Model {
	
	function get_tbl_trayek_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (sippadu_trayek.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR sippadu_trayek.Nm_Usaha like "%'.$keyword.'%" OR sippadu_trayek.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		sippadu_trayek.ID,
		sippadu_trayek.ID_Reg,
		sippadu_trayek.ID_Pelanggan,
		sippadu_trayek.ID_Usaha,
		"" AS Kd_Jns,
		sippadu_trayek.Jenis_Izin,
		sippadu_trayek.Nm_Usaha,
		sippadu_trayek.No_SK,
		sippadu_trayek.Tgl_SK,
		sippadu_trayek.Flow_FO,
		sippadu_trayek.Flow_BO,
		sippadu_trayek.Flow_Kabid,
		sippadu_trayek.Flow_Kadin,
		sippadu_trayek.Flow_TU,
		sippadu_trayek.Flow_Serah,
		sippadu_trayek.Time_Buat,
		sippadu_trayek.Time_Selesai,
		...
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan WHERE sippadu_trayek.Status = 1 AND sippadu_trayek.Flow_FO = 1 and sippadu_trayek.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_trayek_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_trayek_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan WHERE sippadu_trayek.Status = 1 AND sippadu_trayek.Flow_FO = 1 and sippadu_trayek.Flow_BO = 1 and sippadu_trayek.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_trayek_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan WHERE sippadu_trayek.Status = 1 AND sippadu_trayek.Flow_FO = 1 and sippadu_trayek.Flow_BO = 1 and sippadu_trayek.Flow_Kabid = 1 and sippadu_trayek.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_trayek_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan WHERE sippadu_trayek.Status = 1 AND sippadu_trayek.Flow_FO = 1 and sippadu_trayek.Flow_BO = 1 and sippadu_trayek.Flow_Kabid = 1 and sippadu_trayek.Flow_Kadin = 1 and sippadu_trayek.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_trayek_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan WHERE sippadu_trayek.Status = 1 AND sippadu_trayek.Flow_FO = 1 and sippadu_trayek.Flow_BO = 1 and sippadu_trayek.Flow_Kabid = 1 and sippadu_trayek.Flow_Kadin = 1 and sippadu_trayek.Flow_TU = 1 and sippadu_trayek.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_trayek_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		sippadu_trayek.Time_Buat,
		sippadu_trayek.Time_Selesai,
		sippadu_trayek.Nm_Pengambil,
		sippadu_trayek.Time_Diambil,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		tbpelanggan.PROVINSI,
		tbpelanggan.KOTA,
	...
	function get_tbl_trayek_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan WHERE sippadu_trayek.Status = 1 AND sippadu_trayek.Flow_FO = 1 and sippadu_trayek.Flow_BO = 0 and sippadu_trayek.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_trayek($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_trayek');
		return $a;
	}
	
	function delete_tbl_trayek(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('sippadu_trayek',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('sippadu_trayek',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_trayek($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('sippadu_trayek',$in,$wh);
			}
	...
	function insert_trayek_fo(){
		...
		$this->form_validation->set_rules('npwp_usaha','NPWP Usaha','');
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('sippadu_trayek');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_trayek');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/TRAYEK/'.TimeNow('Y');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('sippadu_trayek',$in2);
	...
	function update_trayek($id=0){
		...
		if($Kd_Access == 'bo'){
			...
			$this->form_validation->set_rules('ket_lampiran','Keterangan Lampiran',''); 
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_trayek');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['Ket_Lampiran'] = $this->input->post('ket_lampiran');
			...
			$a = $this->db->update('sippadu_trayek',$in2,$wh2);
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('sippadu_trayek');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_trayek');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_trayek');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('sippadu_trayek');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('sippadu_trayek');
			$dd = $d->first_row();
	...
	function tbl_print($start='',$end=''){
		...
		$a = $this->db->query('SELECT
		sippadu_trayek.ID_Reg AS ID_IZIN,
		sippadu_trayek.Time_Buat AS TGL_BUAT,
		"Izin Tenaga Kesehatan" AS NM_IZIN,
		"" AS KODEJENIS,
		right(sippadu_trayek.Jenis_Izin,1) AS JENIS_IZIN,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		...
		sippadu_trayek.Nm_Usaha AS NM_PERUSAHAAN,
		tbpelanggan.NO_HP,
		0 AS INVESTASI,
		0 AS NAKER_L,
		0 AS NAKER_P,
		sippadu_trayek.Nm_FO,
		sippadu_trayek.Nm_BO,
		sippadu_trayek.Nm_Pengambil,
		sippadu_trayek.Nm_Pengurus AS NM_PENGURUS,
		sippadu_trayek.Time_Selesai AS TGL_SELESAI,
		sippadu_trayek.Time_Diambil AS TGL_DIAMBIL
		FROM
		sippadu_trayek
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_trayek.ID_Pelanggan
		/* INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = sippadu_trayek.ID_Usaha */
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM sippadu_trayek WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM sippadu_trayek WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM sippadu_trayek a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('sippadu_trayek',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
