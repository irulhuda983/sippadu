...
class Mod_itkesehatan extends CI_Model {
	...
	function get_tbl_itkesehatan_fo($status=0,$limit=0,$offset=0){
		...
		FROM
		tbitkesehatan
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbitkesehatan.ID_Pelanggan WHERE tbitkesehatan.Status = 1 AND tbitkesehatan.Flow_FO = 1 and tbitkesehatan.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_itkesehatan_kabid($status=0,$limit=0,$offset=0){
		...
		FROM
		tbitkesehatan
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbitkesehatan.ID_Pelanggan WHERE tbitkesehatan.Status = 1 AND tbitkesehatan.Flow_FO = 1 and tbitkesehatan.Flow_BO = 1 and tbitkesehatan.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_itkesehatan_kadin($status=0,$limit=0,$offset=0){
		...
		FROM
		tbitkesehatan
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbitkesehatan.ID_Pelanggan WHERE tbitkesehatan.Status = 1 AND tbitkesehatan.Flow_FO = 1 and tbitkesehatan.Flow_BO = 1 and tbitkesehatan.Flow_Kabid = 1 and tbitkesehatan.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_itkesehatan_tu($status=0,$limit=0,$offset=0){
		...
		FROM
		tbitkesehatan
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbitkesehatan.ID_Pelanggan WHERE tbitkesehatan.Status = 1 AND tbitkesehatan.Flow_FO = 1 and tbitkesehatan.Flow_BO = 1 and tbitkesehatan.Flow_Kabid = 1 and tbitkesehatan.Flow_Kadin = 1 and tbitkesehatan.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_itkesehatan_cetak($status=0,$limit=0,$offset=0){
		...
		FROM
		tbitkesehatan
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbitkesehatan.ID_Pelanggan WHERE tbitkesehatan.Status = 1 AND tbitkesehatan.Flow_FO = 1 and tbitkesehatan.Flow_BO = 1 and tbitkesehatan.Flow_Kabid = 1 and tbitkesehatan.Flow_Kadin = 1 and tbitkesehatan.Flow_TU = 1 and tbitkesehatan.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_itkesehatan_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbitkesehatan
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbitkesehatan.ID_Pelanggan WHERE tbitkesehatan.Status = 1 AND tbitkesehatan.Flow_FO = 1 and tbitkesehatan.Flow_BO = 0 and tbitkesehatan.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function insert_itkesehatan_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/ITKESEHATAN/'.TimeNow('Y');
	...
	function op_kategori_izin(){
		$h = array();
		$h['0'] = '-';
		$h['1'] = 'SURAT IJIN PRAKTEK BIDAN (SIPB)';
		$h['2'] = 'SURAT IJIN PRAKTEK PERAWAT (SIPP)';
		return $h;
	}
	...
	function for_replace($id=0){
		...
		$kode = '3'.ifnull($aa->Kategori,0);
		$kode2 = '2'.ifnull($aa->Kategori,0);
		$h = array(
