...
class Mod_iujk extends CI_Model {
	...
	function get_tbl_iujk_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbiujk
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbiujk.ID_Pelanggan WHERE tbiujk.Status = 1 AND tbiujk.Flow_FO = 1 and tbiujk.Flow_BO = 0 and tbiujk.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function update_iujk($id=0){
		...
		if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
			...
			$this->form_validation->set_rules('klasifikasi_detail','Klasifikasi Detail','');
		...
		if($this->form_validation->run() == TRUE){
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['Klasifikasi_Detail'] = $this->input->post('klasifikasi_detail');
