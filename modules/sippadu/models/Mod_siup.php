...
class Mod_siup extends CI_Model {
	...
	function get_tbl_siup_fo($status=0,$limit=0,$offset=0){
		...
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbsiup.IDPELANGGAN WHERE tbsiup.Status = 1 AND tbsiup.FLOW = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_siup_kabid($status=0,$limit=0,$offset=0){
		...
		tbsiup
		INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = tbsiup.IDPERUSAHAAN
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbsiup.IDPELANGGAN WHERE tbsiup.Status = 1 AND tbsiup.FLOWA = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_siup_kadin($status=0,$limit=0,$offset=0){
		...
		tbsiup
		INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = tbsiup.IDPERUSAHAAN
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbsiup.IDPELANGGAN WHERE tbsiup.Status = 1 AND tbsiup.FLOWA = "1" and tbsiup.FLOWB = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_siup_tu($status=0,$limit=0,$offset=0){
		...
		tbsiup
		INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = tbsiup.IDPERUSAHAAN
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbsiup.IDPELANGGAN WHERE tbsiup.Status = 1 AND tbsiup.FLOWA = "1" and tbsiup.FLOWB = "1" and tbsiup.FLOWC = "'.$status.'" and tbsiup.FLOWD = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_siup_cetak($status=0,$limit=0,$offset=0){
		...
		tbsiup
		INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = tbsiup.IDPERUSAHAAN
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbsiup.IDPELANGGAN WHERE tbsiup.Status = 1 AND tbsiup.FLOWA = 1 and tbsiup.FLOWB = 1 and tbsiup.FLOWC = 1 and tbsiup.FLOWD = 1 AND tbsiup.FLOWE = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_siup_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		tbsiup
		INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = tbsiup.IDPERUSAHAAN
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbsiup.IDPELANGGAN WHERE tbsiup.Status = 1 and tbsiup.FLOW = 0 AND tbsiup.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function insert_siup_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('tbsiup');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbsiup');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['IDSIUP'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/SIUP/'.TimeNow('Y');
	...
	function get_id_sk($idizin=0){
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbsiup');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM tbsiup WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT NM_PENGURUS AS NM_PENGURUS, ALAMAT_PENGURUS AS ALAMAT_PENGURUS, TELP_PENGURUS AS TELP_PENGURUS FROM tbsiup WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM tbsiup a INNER JOIN tbpelanggan b ON a.IDPELANGGAN = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
