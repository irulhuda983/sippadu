...
class Mod_tdi extends CI_Model {
	
	function get_tbl_tdi_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (tbtdi.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR tbtdi.Nm_Usaha like "%'.$keyword.'%" OR tbtdi.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		FROM
		tbtdi
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdi.ID_Pelanggan WHERE tbtdi.Status = 1 AND tbtdi.Flow_FO = 1 and tbtdi.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdi_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_tdi_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdi
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdi.ID_Pelanggan WHERE tbtdi.Status = 1 AND tbtdi.Flow_FO = 1 and tbtdi.Flow_BO = 1 and tbtdi.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdi_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdi
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdi.ID_Pelanggan WHERE tbtdi.Status = 1 AND tbtdi.Flow_FO = 1 and tbtdi.Flow_BO = 1 and tbtdi.Flow_Kabid = 1 and tbtdi.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdi_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdi
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdi.ID_Pelanggan WHERE tbtdi.Status = 1 AND tbtdi.Flow_FO = 1 and tbtdi.Flow_BO = 1 and tbtdi.Flow_Kabid = 1 and tbtdi.Flow_Kadin = 1 and tbtdi.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdi_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdi
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdi.ID_Pelanggan WHERE tbtdi.Status = 1 AND tbtdi.Flow_FO = 1 and tbtdi.Flow_BO = 1 and tbtdi.Flow_Kabid = 1 and tbtdi.Flow_Kadin = 1 and tbtdi.Flow_TU = 1 and tbtdi.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_tdi_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_tdi_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbtdi
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbtdi.ID_Pelanggan WHERE tbtdi.Status = 1 AND tbtdi.Flow_FO = 1 and tbtdi.Flow_BO = 0 and tbtdi.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tdi($id=0){
		$a = $this->db->where('ID',$id)->get('tbtdi');
		return $a;
	}
	
	function delete_tbl_tdi(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('tbtdi',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('tbtdi',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_tdi($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
	...
	function insert_tdi_fo(){
		...
		$this->form_validation->set_rules('telp_usaha','Telepon Perusahaan','');
		
		if($this->form_validation->run() == TRUE){
			
			$q_last_id = $this->db->select_max('ID','ID')->get('tbtdi');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdi');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/TDI/'.TimeNow('Y');
	...
	function update_tdi($id=0){
		...
		if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
			$this->form_validation->set_rules('npwp','NPWP Perusahaan','');
			$this->form_validation->set_rules('nipik','NIPIK Perusahaan','');
			$this->form_validation->set_rules('lokasi','Lokasi Perusahaan','');
			$this->form_validation->set_rules('op_provinsi2','Provinsi Perusahaan','is_natural_no_zero');
			$this->form_validation->set_rules('op_provinsi2','Kota Perusahaan','is_natural_no_zero');
			$this->form_validation->set_rules('op_kecamatan2','Kecamatan Perusahaan','is_natural_no_zero');
			$this->form_validation->set_rules('op_desa2','Desa Perusahaan','is_natural_no_zero');
			...
			$this->form_validation->set_rules('klui','KLUI','');
			$this->form_validation->set_rules('komoditi','Komoditi','');
			$this->form_validation->set_rules('mesin_utama','Mesin Utama','');
			$this->form_validation->set_rules('mesin_pembantu','Mesin Pembantu','');
			$this->form_validation->set_rules('tenaga_penggerak','Tenaga Penggerak','');
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdi');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			$wh2['ID'] = $id;
			//$in2['IDTDI'] = str_pad($last_id,5,0,STR_PAD_LEFT).'/TDI/'.TimeNow('Y');
			$in2['No_Surat_Mohon'] = $this->input->post('no_mohon');
			$in2['Tgl_Mohon'] = db_time($this->input->post('tgl_mohon'));
			$in2['ID_Pelanggan'] = $this->input->post('idpelanggan');
			$in2['Kd_Pengurus'] = $this->input->post('op_pengurus_permohonan');
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['NIPIK'] = $this->input->post('nipik');
				$in2['Lokasi'] = $this->input->post('lokasi');
				...
				$in2['KLUI'] = $this->input->post('klui');
				$in2['Komoditi'] = $this->input->post('komoditi');
				$in2['Mesin_Utama'] = $this->input->post('mesin_utama');
				$in2['Mesin_Pembantu'] = $this->input->post('mesin_pembantu');
				$in2['Tenaga_Penggerak'] = $this->input->post('tenaga_penggerak');
				$in2['Nilai_Modal'] = $this->input->post('nilai_modal');
			...
			$a = $this->db->update('tbtdi',$in2,$wh2);
	...
	function op_kategori_tdi(){
		$h = array();
		$h['0'] = '- Pilih Kategori TDI -';
		$h['1'] = 'TDI Mikro';
		$h['2'] = 'TDI Kecil';
		$h['3'] = 'TDI Menengah';
		$h['4'] = 'TDI Besar';
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('tbtdi');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdi');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbtdi');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbtdi');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('tbtdi');
			$dd = $d->first_row();
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM tbtdi WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM tbtdi WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM tbtdi a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
