...
class Mod_limbah extends CI_Model {
	...
	function get_tbl_limbah_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (sippadu_limbah.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR sippadu_limbah.Nm_Usaha like "%'.$keyword.'%" OR sippadu_limbah.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		sippadu_limbah.ID,
		sippadu_limbah.ID_Reg,
		sippadu_limbah.ID_Pelanggan,
		sippadu_limbah.ID_Usaha,
		"" AS Kd_Jns,
		sippadu_limbah.Jenis_Izin,
		sippadu_limbah.Nm_Usaha,
		sippadu_limbah.No_SK,
		sippadu_limbah.Tgl_SK,
		sippadu_limbah.Flow_FO,
		sippadu_limbah.Flow_BO,
		sippadu_limbah.Flow_Kabid,
		sippadu_limbah.Flow_Kadin,
		sippadu_limbah.Flow_TU,
		sippadu_limbah.Flow_Serah,
		sippadu_limbah.Time_Buat,
		sippadu_limbah.Time_Selesai,
		...
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan WHERE sippadu_limbah.Status = 1 AND sippadu_limbah.Flow_FO = 1 and sippadu_limbah.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_limbah_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_limbah_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan WHERE sippadu_limbah.Status = 1 AND sippadu_limbah.Flow_FO = 1 and sippadu_limbah.Flow_BO = 1 and sippadu_limbah.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_limbah_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan WHERE sippadu_limbah.Status = 1 AND sippadu_limbah.Flow_FO = 1 and sippadu_limbah.Flow_BO = 1 and sippadu_limbah.Flow_Kabid = 1 and sippadu_limbah.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_limbah_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan WHERE sippadu_limbah.Status = 1 AND sippadu_limbah.Flow_FO = 1 and sippadu_limbah.Flow_BO = 1 and sippadu_limbah.Flow_Kabid = 1 and sippadu_limbah.Flow_Kadin = 1 and sippadu_limbah.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_limbah_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan WHERE sippadu_limbah.Status = 1 AND sippadu_limbah.Flow_FO = 1 and sippadu_limbah.Flow_BO = 1 and sippadu_limbah.Flow_Kabid = 1 and sippadu_limbah.Flow_Kadin = 1 and sippadu_limbah.Flow_TU = 1 and sippadu_limbah.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_limbah_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		sippadu_limbah.Time_Buat,
		sippadu_limbah.Time_Selesai,
		sippadu_limbah.Nm_Pengambil,
		sippadu_limbah.Time_Diambil,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		tbpelanggan.PROVINSI,
		tbpelanggan.KOTA,
	...
	function get_tbl_limbah_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan WHERE sippadu_limbah.Status = 1 AND sippadu_limbah.Flow_FO = 1 and sippadu_limbah.Flow_BO = 0 and sippadu_limbah.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_limbah($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_limbah');
		return $a;
	}
	
	function delete_tbl_limbah(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('sippadu_limbah',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('sippadu_limbah',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_limbah($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('sippadu_limbah',$in,$wh);
			}
	...
	function insert_limbah_fo(){
		...
		$this->form_validation->set_rules('jenis_usaha','Jenis Usaha','');
		$this->form_validation->set_rules('jabatan_pemohon','Jabatan Pemohon','');
		$this->form_validation->set_rules('penanggungjawab','Penanggung Jawab','');
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('sippadu_limbah');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_limbah');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/LIMBAH/'.TimeNow('Y');
			...
			$in2['Jabatan_Pemohon'] = $this->input->post('jabatan_pemohon');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('sippadu_limbah',$in2);
	...
	function update_limbah($id=0){
		...
		if($Kd_Access == 'bo'){
			...
			$this->form_validation->set_rules('no_suratlh','No Surat LH',''); 
			$this->form_validation->set_rules('tgl_suratlh','Tanggal Surat LH',''); 
			$this->form_validation->set_rules('perihal_suratlh','Perihal Surat LH',''); 
			$this->form_validation->set_rules('tembusan','Tembusan',''); 
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_limbah');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['No_SuratLH'] = $this->input->post('no_suratlh');
				$in2['Tgl_SuratLH'] = $this->input->post('tgl_suratlh');
				$in2['Perihal_SuratLH'] = $this->input->post('perihal_suratlh');
			...
			$a = $this->db->update('sippadu_limbah',$in2,$wh2);
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('sippadu_limbah');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_limbah');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_limbah');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('sippadu_limbah');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('sippadu_limbah');
			$dd = $d->first_row();
	...
	function tbl_print($start='',$end=''){
		...
		$a = $this->db->query('SELECT
		sippadu_limbah.ID_Reg AS ID_IZIN,
		sippadu_limbah.Time_Buat AS TGL_BUAT,
		"Izin Tenaga Kesehatan" AS NM_IZIN,
		"" AS KODEJENIS,
		right(sippadu_limbah.Jenis_Izin,1) AS JENIS_IZIN,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		...
		sippadu_limbah.Nm_Usaha AS NM_PERUSAHAAN,
		tbpelanggan.NO_HP,
		0 AS INVESTASI,
		0 AS NAKER_L,
		0 AS NAKER_P,
		sippadu_limbah.Nm_FO,
		sippadu_limbah.Nm_BO,
		sippadu_limbah.Nm_Pengambil,
		sippadu_limbah.Nm_Pengurus AS NM_PENGURUS,
		sippadu_limbah.Time_Selesai AS TGL_SELESAI,
		sippadu_limbah.Time_Diambil AS TGL_DIAMBIL
		FROM
		sippadu_limbah
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_limbah.ID_Pelanggan
		/* INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = sippadu_limbah.ID_Usaha */
	...
	function for_replace($id=0){
		...
		$h = array(
				...
				'[JENISUSAHA]'			=> $aa->Jenis_Usaha,
				'[PERIHAL]'				=> $aa->Perihal,
				'[JABATANPELANGGAN]'				=> $aa->Jabatan_Pemohon,
				'[ALAMATUSAHA]'			=> ($aa->Alamat_Usaha != '' ? $aa->Alamat_Usaha.' - '.desa($aa->Kd_Desa).'<br/>'.kecamatan($aa->Kd_Kecamatan).' '.kota($aa->Kd_Kota) : desa($aa->Kd_Desa).' '.kecamatan($aa->Kd_Kecamatan).' '.kota($aa->Kd_Kota)),
				'[TGLMOHON]'			=> Date_Indo(TimeFormat($aa->Tgl_Mohon,'d F Y')),
				'[NOSURATLH]'				=> $aa->No_SuratLH,
				'[TGLSURATLH]'			=> Date_Indo(TimeFormat($aa->Tgl_SuratLH,'d F Y')),
				'[PERIHALSURATLH]'				=> $aa->Perihal_SuratLH,
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM sippadu_limbah WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM sippadu_limbah WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM sippadu_limbah a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('sippadu_limbah',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
