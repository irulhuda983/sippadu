...
class Mod_huller extends CI_Model {
	
	function get_tbl_huller_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (sippadu_huller.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR sippadu_huller.Nm_Usaha like "%'.$keyword.'%" OR sippadu_huller.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		sippadu_huller.ID,
		sippadu_huller.ID_Reg,
		sippadu_huller.ID_Pelanggan,
		sippadu_huller.ID_Usaha,
		"" AS Kd_Jns,
		sippadu_huller.Jenis_Izin,
		sippadu_huller.Nm_Usaha,
		sippadu_huller.No_SK,
		sippadu_huller.Tgl_SK,
		sippadu_huller.Flow_FO,
		sippadu_huller.Flow_BO,
		sippadu_huller.Flow_Kabid,
		sippadu_huller.Flow_Kadin,
		sippadu_huller.Flow_TU,
		sippadu_huller.Flow_Serah,
		sippadu_huller.Time_Buat,
		sippadu_huller.Time_Selesai,
		...
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan WHERE sippadu_huller.Status = 1 AND sippadu_huller.Flow_FO = 1 and sippadu_huller.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_huller_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_huller_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan WHERE sippadu_huller.Status = 1 AND sippadu_huller.Flow_FO = 1 and sippadu_huller.Flow_BO = 1 and sippadu_huller.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_huller_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan WHERE sippadu_huller.Status = 1 AND sippadu_huller.Flow_FO = 1 and sippadu_huller.Flow_BO = 1 and sippadu_huller.Flow_Kabid = 1 and sippadu_huller.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_huller_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan WHERE sippadu_huller.Status = 1 AND sippadu_huller.Flow_FO = 1 and sippadu_huller.Flow_BO = 1 and sippadu_huller.Flow_Kabid = 1 and sippadu_huller.Flow_Kadin = 1 and sippadu_huller.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_huller_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan WHERE sippadu_huller.Status = 1 AND sippadu_huller.Flow_FO = 1 and sippadu_huller.Flow_BO = 1 and sippadu_huller.Flow_Kabid = 1 and sippadu_huller.Flow_Kadin = 1 and sippadu_huller.Flow_TU = 1 and sippadu_huller.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_huller_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		sippadu_huller.Time_Buat,
		sippadu_huller.Time_Selesai,
		sippadu_huller.Nm_Pengambil,
		sippadu_huller.Time_Diambil,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		tbpelanggan.PROVINSI,
		tbpelanggan.KOTA,
	...
	function get_tbl_huller_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan WHERE sippadu_huller.Status = 1 AND sippadu_huller.Flow_FO = 1 and sippadu_huller.Flow_BO = 0 and sippadu_huller.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_huller($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_huller');
		return $a;
	}
	
	function delete_tbl_huller(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('sippadu_huller',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('sippadu_huller',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_huller($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('sippadu_huller',$in,$wh);
			}
	...
	function insert_huller_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('sippadu_huller');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_huller');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/HULLER/'.TimeNow('Y');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('sippadu_huller',$in2);
	...
	function update_huller($id=0){
		...
		if($Kd_Access == 'bo'){
			...
			$this->form_validation->set_rules('keterangan_alat','Keterangan Alat',''); 
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_huller');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['Kode'] = $this->input->post('kode');
				$in2['Kapasitas'] = $this->input->post('kapasitas');
				$in2['Keterangan_Alat'] = $this->input->post('keterangan_alat');
			...
			$a = $this->db->update('sippadu_huller',$in2,$wh2);
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('sippadu_huller');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_huller');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_huller');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('sippadu_huller');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('sippadu_huller');
			$dd = $d->first_row();
	...
	function tbl_print($start='',$end=''){
		...
		$a = $this->db->query('SELECT
		sippadu_huller.ID_Reg AS ID_IZIN,
		sippadu_huller.Time_Buat AS TGL_BUAT,
		"Izin Tenaga Kesehatan" AS NM_IZIN,
		"" AS KODEJENIS,
		right(sippadu_huller.Jenis_Izin,1) AS JENIS_IZIN,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		...
		sippadu_huller.Nm_Usaha AS NM_PERUSAHAAN,
		tbpelanggan.NO_HP,
		0 AS INVESTASI,
		0 AS NAKER_L,
		0 AS NAKER_P,
		sippadu_huller.Nm_FO,
		sippadu_huller.Nm_BO,
		sippadu_huller.Nm_Pengambil,
		sippadu_huller.Nm_Pengurus AS NM_PENGURUS,
		sippadu_huller.Time_Selesai AS TGL_SELESAI,
		sippadu_huller.Time_Diambil AS TGL_DIAMBIL
		FROM
		sippadu_huller
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_huller.ID_Pelanggan
		/* INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = sippadu_huller.ID_Usaha */
	...
	function for_replace($id=0){
		...
		$h = array(
				...
				'[KODE]'				=> $aa->Kode,
				'[KAPASITAS]'			=> $aa->Kapasitas,
				'[KETERANGANALAT]'			=> $aa->Keterangan_Alat,
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM sippadu_huller WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM sippadu_huller WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM sippadu_huller a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('sippadu_huller',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
