...
class Mod_ho extends CI_Model {
	...
	function get_tbl_ho_fo($status=0,$limit=0,$offset=0){
		...
		FROM
		tbho
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbho.ID_Pelanggan WHERE tbho.Status = 1 AND tbho.Flow_FO = 1 and tbho.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_ho_kabid($status=0,$limit=0,$offset=0){
		...
		FROM
		tbho
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbho.ID_Pelanggan WHERE tbho.Status = 1 AND tbho.Flow_FO = 1 and tbho.Flow_BO = 1 and tbho.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_ho_kadin($status=0,$limit=0,$offset=0){
		...
		FROM
		tbho
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbho.ID_Pelanggan WHERE tbho.Status = 1 AND tbho.Flow_FO = 1 and tbho.Flow_BO = 1 and tbho.Flow_Kabid = 1 and tbho.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_ho_tu($status=0,$limit=0,$offset=0){
		...
		FROM
		tbho
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbho.ID_Pelanggan WHERE tbho.Status = 1 AND tbho.Flow_FO = 1 and tbho.Flow_BO = 1 and tbho.Flow_Kabid = 1 and tbho.Flow_Kadin = 1 and tbho.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_ho_cetak($status=0,$limit=0,$offset=0){
		...
		FROM
		tbho
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbho.ID_Pelanggan WHERE tbho.Status = 1 AND tbho.Flow_FO = 1 and tbho.Flow_BO = 1 and tbho.Flow_Kabid = 1 and tbho.Flow_Kadin = 1 and tbho.Flow_TU = 1 and tbho.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_ho_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbho
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbho.ID_Pelanggan WHERE tbho.Status = 1 AND tbho.Flow_FO = 1 and tbho.Flow_BO = 0 and tbho.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function insert_ho_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('tbho');
			$last_id = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/HO/'.TimeNow('Y');
	...
	function get_id_sk($idizin=0){
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbho');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_jenis_izin($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_ho');
		$h = 0;
		if($a->num_rows() > 0){
			$aa = $a->row();
			$h = $aa->Jenis_Izin;
