...
class Mod_bengkel extends CI_Model {
	
	function get_tbl_bengkel_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (sippadu_bengkel.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR sippadu_bengkel.Nm_Usaha like "%'.$keyword.'%" OR sippadu_bengkel.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		sippadu_bengkel.ID,
		sippadu_bengkel.ID_Reg,
		sippadu_bengkel.ID_Pelanggan,
		sippadu_bengkel.ID_Usaha,
		"" AS Kd_Jns,
		sippadu_bengkel.Jenis_Izin,
		sippadu_bengkel.Nm_Usaha,
		sippadu_bengkel.No_SK,
		sippadu_bengkel.Tgl_SK,
		sippadu_bengkel.Flow_FO,
		sippadu_bengkel.Flow_BO,
		sippadu_bengkel.Flow_Kabid,
		sippadu_bengkel.Flow_Kadin,
		sippadu_bengkel.Flow_TU,
		sippadu_bengkel.Flow_Serah,
		sippadu_bengkel.Time_Buat,
		sippadu_bengkel.Time_Selesai,
		...
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan WHERE sippadu_bengkel.Status = 1 AND sippadu_bengkel.Flow_FO = 1 and sippadu_bengkel.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_bengkel_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_bengkel_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan WHERE sippadu_bengkel.Status = 1 AND sippadu_bengkel.Flow_FO = 1 and sippadu_bengkel.Flow_BO = 1 and sippadu_bengkel.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_bengkel_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan WHERE sippadu_bengkel.Status = 1 AND sippadu_bengkel.Flow_FO = 1 and sippadu_bengkel.Flow_BO = 1 and sippadu_bengkel.Flow_Kabid = 1 and sippadu_bengkel.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_bengkel_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan WHERE sippadu_bengkel.Status = 1 AND sippadu_bengkel.Flow_FO = 1 and sippadu_bengkel.Flow_BO = 1 and sippadu_bengkel.Flow_Kabid = 1 and sippadu_bengkel.Flow_Kadin = 1 and sippadu_bengkel.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_bengkel_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan WHERE sippadu_bengkel.Status = 1 AND sippadu_bengkel.Flow_FO = 1 and sippadu_bengkel.Flow_BO = 1 and sippadu_bengkel.Flow_Kabid = 1 and sippadu_bengkel.Flow_Kadin = 1 and sippadu_bengkel.Flow_TU = 1 and sippadu_bengkel.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_bengkel_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		sippadu_bengkel.Time_Buat,
		sippadu_bengkel.Time_Selesai,
		sippadu_bengkel.Nm_Pengambil,
		sippadu_bengkel.Time_Diambil,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		tbpelanggan.PROVINSI,
		tbpelanggan.KOTA,
	...
	function get_tbl_bengkel_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan WHERE sippadu_bengkel.Status = 1 AND sippadu_bengkel.Flow_FO = 1 and sippadu_bengkel.Flow_BO = 0 and sippadu_bengkel.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_bengkel($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_bengkel');
		return $a;
	}
	
	function delete_tbl_bengkel(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('sippadu_bengkel',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('sippadu_bengkel',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_bengkel($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('sippadu_bengkel',$in,$wh);
			}
	...
	function insert_bengkel_fo(){
		...
		$this->form_validation->set_rules('jenis_usaha','Lokasi Usaha','');
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('sippadu_bengkel');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_bengkel');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/BENGKEL/'.TimeNow('Y');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('sippadu_bengkel',$in2);
	...
	function update_bengkel($id=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_bengkel');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			$a = $this->db->update('sippadu_bengkel',$in2,$wh2);
	...
	function get_data_bengkel($id=0){
		$a = $this->db->where('ID',$id)->get('sippadu_bengkel');
		return $a;
	}
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('sippadu_bengkel');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_bengkel');
		$h = '';
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_bengkel');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('sippadu_bengkel');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('sippadu_bengkel');
			$dd = $d->first_row();
	...
	function tbl_print($start='',$end=''){
		...
		$a = $this->db->query('SELECT
		sippadu_bengkel.ID_Reg AS ID_IZIN,
		sippadu_bengkel.Time_Buat AS TGL_BUAT,
		"Izin Tenaga Kesehatan" AS NM_IZIN,
		"" AS KODEJENIS,
		right(sippadu_bengkel.Jenis_Izin,1) AS JENIS_IZIN,
		tbpelanggan.NM_PELANGGAN,
		tbpelanggan.ALAMATPELANGGAN,
		...
		sippadu_bengkel.Nm_Usaha AS NM_PERUSAHAAN,
		tbpelanggan.NO_HP,
		0 AS INVESTASI,
		0 AS NAKER_L,
		0 AS NAKER_P,
		sippadu_bengkel.Nm_FO,
		sippadu_bengkel.Nm_BO,
		sippadu_bengkel.Nm_Pengambil,
		sippadu_bengkel.Nm_Pengurus AS NM_PENGURUS,
		sippadu_bengkel.Time_Selesai AS TGL_SELESAI,
		sippadu_bengkel.Time_Diambil AS TGL_DIAMBIL
		FROM
		sippadu_bengkel
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = sippadu_bengkel.ID_Pelanggan
		/* INNER JOIN tbperusahaan ON tbperusahaan.IDPERUSAHAAN = sippadu_bengkel.ID_Usaha */
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM sippadu_bengkel WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM sippadu_bengkel WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM sippadu_bengkel a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('sippadu_bengkel',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
