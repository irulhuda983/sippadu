...
class Mod_reklame extends CI_Model {
	...
	function get_tbl_reklame_fo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
		...
		if($this->input->post('keyword') != '' OR $this->input->post('search') != ''){
			...
			$q_where .= 'AND (tbreklame.ID_Pelanggan like "%'.$keyword.'%"  or tbpelanggan.NM_PELANGGAN like "%'.$keyword.'%" OR tbreklame.Nm_Usaha like "%'.$keyword.'%" OR tbreklame.ID_Reg like "%'.$keyword.'%")';
			$q_limit = '';
		}
		...
		FROM
		tbreklame
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbreklame.ID_Pelanggan WHERE tbreklame.Status = 1 AND tbreklame.Flow_FO = 1 and tbreklame.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_reklame_bo($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_reklame_kabid($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbreklame
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbreklame.ID_Pelanggan WHERE tbreklame.Status = 1 AND tbreklame.Flow_FO = 1 and tbreklame.Flow_BO = 1 and tbreklame.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_reklame_kadin($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbreklame
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbreklame.ID_Pelanggan WHERE tbreklame.Status = 1 AND tbreklame.Flow_FO = 1 and tbreklame.Flow_BO = 1 and tbreklame.Flow_Kabid = 1 and tbreklame.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_reklame_tu($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbreklame
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbreklame.ID_Pelanggan WHERE tbreklame.Status = 1 AND tbreklame.Flow_FO = 1 and tbreklame.Flow_BO = 1 and tbreklame.Flow_Kabid = 1 and tbreklame.Flow_Kadin = 1 and tbreklame.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_reklame_cetak($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbreklame
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbreklame.ID_Pelanggan WHERE tbreklame.Status = 1 AND tbreklame.Flow_FO = 1 and tbreklame.Flow_BO = 1 and tbreklame.Flow_Kabid = 1 and tbreklame.Flow_Kadin = 1 and tbreklame.Flow_TU = 1 and tbreklame.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	
	function get_tbl_reklame_serah($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
	...
	function get_tbl_reklame_rekom($status=0,$limit=0,$offset=0){
		$q_limit = '';
		$q_where = '';
		if($status==1){
			$q_order = 'ORDER BY ID DESC';
		...
		FROM
		tbreklame
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbreklame.ID_Pelanggan WHERE tbreklame.Status = 1 AND tbreklame.Flow_FO = 1 and tbreklame.Flow_BO = 0 and tbreklame.Flow_Rekom = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function delete_tbl_reklame(){
		...
		if(is_array($chk)){
			...
			foreach($chk as $id){
				...
				$wh['Flow_TU'] = '0';
				$wh['Flow_Serah'] = '0';
				$a = $this->db->get_where('tbreklame',$wh);
				if($a->num_rows() > 0){
					$b = $this->db->delete('tbreklame',$wh);
					$count_success[] = 1; 
				}
				else{
					$count_error[] = 1;
	...
	function verifikasi_tbl_reklame($param='',$status=1){
		$chk = $this->input->post('chk');
		if(is_array($chk)){
			foreach($chk as $id){
				$wh['ID'] = $id;
				...
				$this->db->update('tbreklame',$in,$wh);
			}
	...
	function insert_reklame_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$q_last_id = $this->db->select_max('ID','ID')->get('tbreklame');
			$last_id = 1;
			...
			$q_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbreklame');
			$last_reg = 1;
			...
			$in2['No_Reg'] = $last_reg;
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_reg,5,0,STR_PAD_LEFT).'/REKLAME/'.TimeNow('Y');
			...
			$in2['Status'] = 1;
			$a = $this->db->insert('tbreklame',$in2);
	...
	function update_reklame($id=0){
		...
		if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
			...
			$this->form_validation->set_rules('alamat_usaha','Alamat Usaha','required');
			$this->form_validation->set_rules('perihal','Pemasangan Untuk','required');
			$this->form_validation->set_rules('naskah','Naskah Reklame','required');
			$this->form_validation->set_rules('ukuran','Ukuran Reklame','required');
			$this->form_validation->set_rules('pasang_awal','Pemasangan Awal','required');
			$this->form_validation->set_rules('pasang_akhir','Pemasangan Akhir','required');
			$this->form_validation->set_rules('lokasi','Lokasi Usaha','');
			//$this->form_validation->set_rules('op_provinsi','Provinsi','is_natural_no_zero');
			//$this->form_validation->set_rules('op_provinsi','Kota','is_natural_no_zero');
			//$this->form_validation->set_rules('op_kecamatan','Kecamatan','is_natural_no_zero');
			...
			$this->form_validation->set_rules('sk_tembusan','Tembusan','');
		...
		if($this->form_validation->run() == TRUE){
			...
			/*$q_last_id = $this->db->select_max('ID','ID')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbreklame');
			$last_id = 1;
			if($q_last_id->num_rows() > 0){
				$a_last_id = $q_last_id->row();
				$last_id = $a_last_id->ID + 1;
			...
			$wh2['ID'] = $id;
			//$in2['IDREKLAME'] = str_pad($last_id,5,0,STR_PAD_LEFT).'/REKLAME/'.TimeNow('Y');
			$in2['No_Surat_Mohon'] = $this->input->post('no_mohon');
			$in2['Tgl_Mohon'] = db_time($this->input->post('tgl_mohon'));
			$in2['ID_Pelanggan'] = $this->input->post('idpelanggan');
			$in2['Kd_Pengurus'] = $this->input->post('op_pengurus_permohonan');
			...
			if($Kd_Access == 'bo' || $Kd_Access == 'tu'){
				...
				$in2['Alamat_Usaha'] = $this->input->post('alamat_usaha');
				$in2['Perihal'] = $this->input->post('perihal');
				$in2['Naskah_Reklame'] = $this->input->post('naskah');
				$in2['Ukuran_Reklame'] = $this->input->post('ukuran');
				$in2['J_Waktu_Awal'] = db_time($this->input->post('pasang_awal'));
				$in2['J_Waktu_Akhir'] = db_time($this->input->post('pasang_akhir'));
				$in2['Lokasi'] = $this->input->post('lokasi');
				//$in2['Kd_Provinsi'] = $this->input->post('op_provinsi');
				//$in2['Kd_Kota'] = $this->input->post('op_kota');
				//$in2['Kd_Kecamatan'] = $this->input->post('op_kecamatan');
			...
			$a = $this->db->update('tbreklame',$in2,$wh2);
	...
	function get_data_reklame($id=0){
		$a = $this->db->where('ID',$id)->get('tbreklame');
		return $a;
	}
	...
	function op_kategori_reklame(){
		$h = array();
		$h['0'] = '- Pilih Kategori REKLAME -';
		$h['1'] = 'REKLAME Mikro';
		$h['2'] = 'REKLAME Kecil';
		$h['3'] = 'REKLAME Menengah';
		$h['4'] = 'REKLAME Besar';
		return $h;
	}
	...
	function get_nomor_sk($idizin=0,$param=0,$kbli=0){
		...
		$a = $this->db->where('ID',$idizin)->get('tbreklame');
		$aa = $a->first_row();
	...
	function get_id_sk($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbreklame');
		$h = 0;
		if($a->num_rows() > 0){
			$aa = $a->first_row();
			$h = $aa->ID_SK;
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbreklame');
			$h = 1;
			if($d->num_rows() > 0){
				$dd = $d->first_row();
				$h = $dd->ID_SK+1;
	...
	function get_id_agenda($idizin=0){
		$a = $this->db->where('ID',$idizin)->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbreklame');
		$aa = $a->first_row();
		$h = $aa->ID_AGENDA;
		
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_AGENDA','ID_AGENDA')->get('tbreklame');
			$dd = $d->first_row();
	...
	function data_serah($idizin=0){
		$a = $this->db->query('SELECT IFNULL(Kd_Pengambil,0) AS KD_PENGAMBIL, Nm_Pengambil AS NM_PENGAMBIL, Alamat_Pengambil AS ALAMAT_PENGAMBIL, Telp_Pengambil AS TELP_PENGAMBIL FROM tbreklame WHERE ID = '.$idizin);
		return $a;
	}
	...
	function data_serah_ajax(){
		...
		switch($kdpengambil){
			case 1:
				$q = $this->db->query('SELECT Nm_Pengurus AS NM_PENGURUS, Alamat_Pengurus AS ALAMAT_PENGURUS, Telp_Pengurus AS TELP_PENGURUS FROM tbreklame WHERE ID = '.$idizin);
				if($q->num_rows() > 0){
					$get = $q->row();
					...
					if($nm_pengurus != ''){
						...
						$a = $this->db->query('SELECT b.NM_PELANGGAN AS NM_PENGURUS, b.ALAMATPELANGGAN AS ALAMAT_PENGURUS, REPLACE(NO_HP,"+62","0") AS TELP_PENGURUS FROM tbreklame a INNER JOIN tbpelanggan b ON a.ID_Pelanggan = b.IDPELANGGAN WHERE a.ID = '.$idizin);
					}
				}
				break;
			case 2:
	...
	function simpan_pengambil($idizin=0){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in['Time_Diambil'] = TimeNow('Y-m-d H:i');
			$a = $this->db->update('tbreklame',$in,$wh);
			if($a){
				set_alert('success','Data berhasil disimpan');
