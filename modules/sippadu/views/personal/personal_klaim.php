								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<!--<input class="btn btn-blue" type="button" name="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('personal/klaim_add')); ?>'"/>-->
														...
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																...
																<tr>                                    
																	...
																	<td>'.$d->No_Kitas.'</td>
																	<td>'.$d->Nm_Personal.'</td>
																	<td>'.($d->Tempat_Lahir != '' ? $d->Tempat_Lahir.', ' : '').' '.TimeFormat($d->Tgl_Lahir,'d-m-Y').'</td>
																	<td>'.$d->No_HP.'</td>
																	<td>'.anchor_popup_label(base_url('assets/'.config('theme').'/files/berkas/personal/'.$d->Filename),'Dokumen','info',$popup).form_hidden('file-'.$d->ID,$d->Filename).'</td> 
																	<td align="center">'.anchor_label(site_url(fmodule('personal/klaim_detail/'.$d->ID)),'Detail','success').'</td>
																</tr>  '; 
															}
														}
													?>
