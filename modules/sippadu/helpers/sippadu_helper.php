...
function cari_history_limbah($id=0){
	$CI =& get_instance();
	$a = $CI->db->query('SELECT
	a.ID_Pelanggan,
	a.ID_Usaha,
	...
	a.Nm_Usaha
	FROM
	sippadu_limbah a WHERE a.ID_Pelanggan = "'.$id.'" ORDER BY a.ID_Pelanggan ASC');
	$h = '';
	if($a->num_rows() > 0){
		foreach($a->result() as $aa){
			$h .= '
			<tr class="success" style="">
				...
				if($aa->Flow_Serah == '1'){
					$h .= '
					<td align="center">'.anchor_label(fmodule('limbah/fo_daftar/2/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Dafta Ulang','success').''.anchor_label(fmodule('limbah/fo_daftar/3/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Perubahan','error').'</td>';
				}
				else{
					$h .= '
					<td align="center">'.label('warning','Dalam Proses').'</td>';
...
function cari_history_koperasi($id=0){
	$CI =& get_instance();
	$a = $CI->db->query('SELECT
	a.ID_Pelanggan,
	a.ID_Usaha,
	...
	a.Nm_Usaha
	FROM
	sippadu_koperasi a WHERE a.ID_Pelanggan = "'.$id.'" ORDER BY a.ID_Pelanggan ASC');
	$h = '';
	if($a->num_rows() > 0){
		foreach($a->result() as $aa){
			$h .= '
			<tr class="success" style="">
				...
				if($aa->Flow_Serah == '1'){
					$h .= '
					<td align="center">'.anchor_label(fmodule('koperasi/fo_daftar/2/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Dafta Ulang','success').''.anchor_label(fmodule('koperasi/fo_daftar/3/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Perubahan','error').'</td>';
				}
				else{
					$h .= '
					<td align="center">'.label('warning','Dalam Proses').'</td>';
...
function cari_history_trayek($id=0){
	$CI =& get_instance();
	$a = $CI->db->query('SELECT
	a.ID_Pelanggan,
	a.ID_Usaha,
	...
	a.Nm_Usaha
	FROM
	sippadu_trayek a WHERE a.ID_Pelanggan = "'.$id.'" ORDER BY a.ID_Pelanggan ASC');
	$h = '';
	if($a->num_rows() > 0){
		foreach($a->result() as $aa){
			$h .= '
			<tr class="success" style="">
				...
				if($aa->Flow_Serah == '1'){
					$h .= '
					<td align="center">'.anchor_label(fmodule('trayek/fo_daftar/2/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Dafta Ulang','success').''.anchor_label(fmodule('trayek/fo_daftar/3/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Perubahan','error').'</td>';
				}
				else{
					$h .= '
					<td align="center">'.label('warning','Dalam Proses').'</td>';
...
function cari_history_bengkel($id=0){
	$CI =& get_instance();
	$a = $CI->db->query('SELECT
	a.ID_Pelanggan,
	a.ID_Usaha,
	...
	a.Nm_Usaha
	FROM
	sippadu_bengkel a WHERE a.ID_Pelanggan = "'.$id.'" ORDER BY a.ID_Pelanggan ASC');
	$h = '';
	if($a->num_rows() > 0){
		foreach($a->result() as $aa){
			$h .= '
			<tr class="success" style="">
				...
				if($aa->Flow_Serah == '1'){
					$h .= '
					<td align="center">'.anchor_label(fmodule('bengkel/fo_daftar/2/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Dafta Ulang','success').''.anchor_label(fmodule('bengkel/fo_daftar/3/'.$aa->ID_Pelanggan.'/'.$aa->ID_Usaha),'Perubahan','error').'</td>';
				}
				else{
					$h .= '
					<td align="center">'.label('warning','Dalam Proses').'</td>';
...
function itkes($x=0,$type=0){
	...
	switch($type.$x){
		...
		case '02':
			...
			break;
		case '11':
			$h = 'SURAT IZIN PRAKTEK BIDAN (SIPB)';
			break;
		case '12':
			$h = 'SURAT IZIN PRAKTEK PERAWAT (SIPP)';
			break;
		case '21':
			$h = 'Surat Izin Praktek Bidan (SIPB)';
			break;
...
function print_sk1($Kd_Izin=0){
	$CI =& get_instance();
	$a = $CI->db->where('Kd_Izin',$Kd_Izin)->get('tb_izin');
	$h = 0;
	if($a->num_rows() > 0){
		$aa = $a->row();
		$h = $aa->SK1;
...
function sippadu_rekom($id_izin=0,$s=''){
	$CI =& get_instance();
	//$db_group = $CI->db->groupname;
	//$db = $CI->load->database($db_group,TRUE);
	$a = $CI->db->query('SELECT b.* FROM tb_izin a inner join sippadu_skpd b ON a.SKPD_Rekom = b.Kd_SKPD WHERE a.Kd_Izin = "'.$id_izin.'"');
	$kd_skpd = 0;
	$h = '';
	if($a->num_rows() > 0){
		$aa = $a->first_row();
