...
class Mod_opklinik extends CI_Model {
	...
	function insert_opklinik(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$qc_last_id = $this->db->select_max('ID','ID')->get('sippadu_opklinik');
			$last_id_izin = 1;
			...
			$qc_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('sippadu_opklinik');
			$last_reg_izin = 1;
			...
			$in4['ID'] = $last_id_izin;
			$in4['ID_Reg'] = str_pad($last_reg_izin,5,0,STR_PAD_LEFT).'/OPKLINIK/'.TimeNow('Y');
			...
			$in4['Status'] = 1;
			$a = $this->db->insert('sippadu_opklinik',$in4);
			...
			if($data_dok->num_rows() > 0){ 
				foreach($data_dok->result() as $dok){ 
					...
					if ($_FILES['userfile_'.$Kd_Dok]['name'] != ''){
						$config['upload_path'] 	= $upload_path = './assets/'.config('theme').'/files/berkas/opklinik/';
