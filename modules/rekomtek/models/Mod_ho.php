...
class Mod_ho extends CI_Model {
	...
	function insert_ho(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$qc_last_id = $this->db->select_max('ID','ID')->get('tbho');
			$last_id_izin = 1;
			...
			$qc_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(Time_Buat)',TimeNow('Y'))->get('tbho');
			$last_reg_izin = 1;
			...
			$in4['ID'] = $last_id_izin;
			$in4['ID_Reg'] = str_pad($last_reg_izin,5,0,STR_PAD_LEFT).'/HO/'.TimeNow('Y');
			...
			$in4['Perihal'] = $this->input->post('perihal');
			$in4['Lokasi'] = $this->input->post('lokasi');
			...
			if($this->input->post('jenis_izin') > 1){
			$in4['Nm_Pelanggan_L'] = $this->input->post('nama_l');
			$in4['Nm_Usaha_L'] = $this->input->post('nama_usaha_l');
			$in4['Perihal_L'] = $this->input->post('perihal_l');
			$in4['Alamat_Usaha_L'] = $this->input->post('alamat_usaha_l');
			$in4['Kd_Provinsi_L'] = $this->input->post('op_provinsi2');
			$in4['Kd_Kota_L'] = $this->input->post('op_kota2');
			$in4['Kd_Kecamatan_L'] = $this->input->post('op_kecamatan2');
			$in4['Kd_Desa_L'] = $this->input->post('op_desa2');
			}
			...
			if($data_dok->num_rows() > 0){ 
				foreach($data_dok->result() as $dok){ 
					...
					if ($_FILES['userfile_'.$Kd_Dok]['name'] != ''){
						$config['upload_path'] 	= $upload_path = './assets/'.config('theme').'/files/berkas/ho/';
