...
class Lingkungan extends CI_Controller {
	...
	public function fo_edit($idlingkungan=0)
	{
		...
		$idperusahaan = 0;
		$Kd_Pengurus = 0;
		$data = $this->mod_lingkungan->get_lingkungan($idlingkungan);
		if($data->num_rows() > 0){
			$aa = $data->row();
		...
		if($this->input->post('submit')){
			$this->mod_lingkungan->update_lingkungan($idlingkungan);
		}
		...
		$d['op_bentuk_perusahaan'] = $this->mod_lingkungan->op_bentuk_perusahaan();
		$d['data'] = $this->mod_lingkungan->get_personal($idpelanggan);
		$d['data2'] = $this->mod_lingkungan->get_data_lingkungan($idlingkungan);
		$d['Kd_Pelanggan'] = $idpelanggan;
		$d['Kd_Perusahaan'] = $idperusahaan;
		$d['Kd_Jenis'] = $jenis_izin;
		$d['Kd_Pengurus'] = $Kd_Pengurus;
		$d['ID_Izin'] = $idlingkungan;
	...
	public function bo_edit($idlingkungan=0)
	{
	...
	public function tu_edit($idlingkungan=0)
	{
