...
class Mod_sippadu extends CI_Model {
	...
	function grafik_registrasi($limit=60){
		...
		$a = $this->db->query('CALL sippadu_RptDashboard60Hari("'.AuthIzin().'")');
		$a->next_result(); 
		//$a->free_result(); 
		return $a;
	}
	...
	function pie_hari($limit=100){
		...
		$a = $this->db->query('CALL sippadu_RptIzinRekapDate("d",'.TimeNow('d').')');
		$a->next_result(); 
		return $a;
	}
	...
	function pie_bulan($limit=100){
		...
		$a = $this->db->query('CALL sippadu_RptDashboardBulan("'.AuthIzin().'",'.TimeNow('Y').','.TimeNow('m').')');
		$a->next_result(); 
		return $a;
	}
	function pie_tahun($limit=100){
		...
		$a = $this->db->query('CALL sippadu_RptDashboardTahun("'.AuthIzin().'",'.TimeNow('Y').')');
		$a->next_result(); 
		return $a;
	}
	...
	function notifikasi(){
		$a = $this->db->query('CALL sippadu_RptIzinNotifikasi("'.AuthIzin().'")');
		$a->next_result(); 
		return $a;
	}
