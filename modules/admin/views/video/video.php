													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_Video).'<i></i></label></td>
																	<td>'.create_iframe_video($d->ID_Video).'</td>
																	<td>'.$d->Nm_Video.'</td>
																	<td>'.video_kategori($d->ID_Video).'</td>
																	<td>'.character_limiter($d->Deskripsi,150).'</td>
																	<td>'.$d->Nm_Source.'</td> 
																	<td>'.$d->Time.'</td> 
																	<td>'.$d->Hits.'</td> 
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule($class_page.'/edit/'.$d->ID_Video))).'</td>	
																</tr>  '; 
															}
														}
													?>
