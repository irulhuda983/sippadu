								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule($class_page.'/add')); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_Project).'<i></i></label></td>
																	<td>'.$d->Nm_Project.'</td>
																	<td>'.project_kategori($d->ID_Project).'</td>
																	<td>'.$d->Nm_Instansi.'</td> 
																	<td>'.TimeFormat($d->Time_Mulai,'d-m-Y').'</td> 
																	<td>'.TimeFormat($d->Time_Akhir,'d-m-Y').'</td> 
																	<td>'.$d->Lokasi.'</td> 
																	<td>'.number($d->Anggaran).'</td> 
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule($class_page.'/edit/'.$d->ID_Project))).'</td>	
																</tr>  '; 
															}
														}
													?>
