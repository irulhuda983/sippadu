    ...
    <body id="minovate" class="appWrapper">
									...
									<?php echo form_open_multipart('',array('class' => 'form-horizontal', 'role' => 'form')); ?>
										<div class="block-fluid">                        
											...
											<div class="form-group">
												<label for="" class="col-sm-2 control-label">Nama Form</label>
												<div class="col-sm-10"><?php echo form_input('Nm_Form',set_value('Nm_Form',$d->Nm_Form),'class="form-control"'); ?></div>
											</div>                         
											...
											if($data_kolom->num_rows() > 0){ 
												foreach($data_kolom->result() as $k){
													...
													<input type="text" id="no_'.$nomor.'" name="Nomor[]" value="'.$nomor.'" class="form-control" placeholder="No" /></div><div class="col-sm-4"><input type="text" id="val_'.$nomor.'" name="Kolom[]" value="'.$k->Nm_Col.'"  class="form-control" placeholder="Nama Kolom" /></div><div class="col-sm-2"><input type="text" id="type_'.$nomor.'"name="Type[]" value="'.$k->Nm_Type.'" class="form-control" placeholder="" disabled/></div><div class="col-sm-1"><input type="button" id="setting_'.$nomor.'" class="btn btn-blue" onclick="setting('.$nomor.');" value="Setting" /></div><div class="col-sm-1"><input type="button" id="remove_'.$nomor.'" class="btn btn-red" onclick="remove('.$nomor.');" value="Hapus" /><input type="button" id="undo_'.$nomor.'" class="btn btn-greensea" onclick="undo('.$nomor.');" value="Undo" style="display:none"/></div></div>';
													++$n;
												}
												echo $h;
											}
											...
											<div class="form-group">
												...
												<div class="col-sm-5">
													<?php echo imgsrc($d->Image_Raw.'_thumb'.$d->Image_Ext,'form',config('main_site')); ?>
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-5"><?php echo form_multiselect('op_form_kategori[]',$op_form_kategori,set_value('op_form_kategori[]',$slc_op_form_kategori),'class="form-control chosen-select"'); ?></div>
											</div> 
		...
		window.onload=function(){
			...
			$(add_button).click(function(e){
				...
				if(id <= max_fields){
					$(wrapper).append('<div id="el_'+id+'" class="form-group"><label for="" class="col-sm-2 control-label"></label><div class="col-sm-1"><input type="hidden" id="id_col_'+id+'" name="ID_Col[]" value="0" /><input type="hidden" id="id_'+ id +'" name="ID[]" value="'+id+'" /><input type="text" id="no_'+ id +'" name="Nomor[]" value="'+id+'" class="form-control" placeholder="No" /></div><div class="col-sm-4"><input type="text" id="val_'+ id +'" name="Kolom[]" value=""  class="form-control" placeholder="Nama Kolom" /></div><div class="col-sm-2"><input type="text"  id="type_'+ id +'"name="Type[]" value="Text" class="form-control" placeholder="" disabled/></div><div class="col-sm-1"><input type="button" id="setting_'+id+'" class="btn btn-blue" onclick="setting('+id+');" value="Setting" /></div><div class="col-sm-1"><input type="button" id="remove_'+id+'" class="btn btn-red" onclick="remove('+id+');" value="Hapus" /><input type="button" id="undo_'+id+'" class="btn btn-greensea" onclick="undo('+id+');" value="Undo" style="display:none"/></div></div>');
				}
			});
