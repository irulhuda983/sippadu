								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('gallery/add/'.$album)); ?>'"/>
											...
											<table  class="table table-hover" id="" style="">
												<thead>
													<tr class="active">                                    
														...
														<th width="">Image</th>
														<th width="">Deksripsi</th>
														<th width="150px">Time</th>
														<th width="20px">Uploader</th>
														<th width="20px">Cover</th>
														...
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																...
																<tr>                                    
																	...
																	<td>'.imgsrc($d->Image_Raw.'_thumb'.$d->Image_Ext,'gallery',config('main_site')).form_hidden('Image_Raw'.$d->ID,$d->Image_Raw).form_hidden('Image_Ext'.$d->ID,$d->Image_Ext).'</td> 
																	<td>'.$d->Deskripsi.'</td>
																	<td>'.$d->Time.'</td> 
																	<td>'.$d->Nm_User.'</td>
																	<td>'.($d->Cover==1 ? label('success','Cover') : anchor_label(fmodule('gallery/cover/'.$d->ID_Album.'/'.$d->ID),'Set Cover','error')).'</td>
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule('gallery/edit/'.$album.'/'.$d->ID))).'</td>	
																</tr>  '; 
															}
														}
													?>
