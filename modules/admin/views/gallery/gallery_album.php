								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('gallery/album_add')); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_Album).'<i></i></label></td>
																	<td>'.anchor(site_url(fmodule('gallery/detail/'.$d->ID_Album)),$d->Nm_Album).'</td>
																	<td>'.gallery_kategori($d->ID_Album).'</td>
																	<td>'.$d->Deskripsi.'</td>
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule('gallery/album_edit/'.$d->ID_Album))).'</td>	
																</tr>  '; 
															}
														}
													?>
