		...
		var rkaTable;
		 $("#save_table").click(function(){
			if($("#tSortable").length > 0){
				var oSettings = rkaTable.fnSettings();
				oSettings._iDisplayLength = 1000;
				rkaTable.fnDraw();
				rkaTable = $("#tSortable").dataTable();
				rkaTable.fnFilter('');
			}
			$('<input />').attr('type','hidden').attr('name','save').attr('value','1').appendTo('#form');
			$('#form').submit();
		});
		rkaTable = $("#tSortable").dataTable({"iDisplayLength": 100, "aLengthMenu": [10,25,50,100,500,1000], "sPaginationType": "full_numbers","bSort":false});
		...
		function UpdateUserLog(){
			$.post("<?php echo apps_url(2,fmodule('users/update_user_log/')); ?>?t=" + new Date().getTime());
		}
		
		function CekUserLog(){
			$("#badge_chat").load("<?php echo apps_url(2,fmodule('users/cek_user_log/')); ?>?t=" + new Date().getTime());
		}
		
		function UserListOnline(){
			$("#users_list_online").load("<?php echo apps_url(2,fmodule('users/users_list_online/')); ?>?t=" + new Date().getTime());
		}
