								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('grafik/add')); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																...
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_Grafik).'<i></i></label></td>
																	<td>'.anchor(site_url(fmodule('grafik/data/'.$d->ID_Grafik)),$d->Nm_Grafik).'</td>
																	<td>'.grafik_kategori($d->ID_Grafik).'</td>
																	<td>'.ifnull($d->Deskripsi).'</td>
																	<td>'.$d->Nm_Jenis.'</td>
																	<td>'.$order.', '.$d->Asc_Desc.'</td>
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule('grafik/edit/'.$d->ID_Grafik))).'</td>	
																</tr>  '; 
															}
														}
													?>
