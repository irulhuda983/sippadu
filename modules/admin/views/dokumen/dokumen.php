								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('dokumen/add')); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_Dokumen).'<i></i></label></td>
																	<td>'.$d->Judul.'</td>
																	<td>'.dokumen_kategori($d->ID_Dokumen).form_hidden('File'.$d->ID_Dokumen,$d->File_Name).'</td>
																	<td>'.$d->Nm_User.'</td>
																	<td>'.$d->Time.'</td> 
																	<td>'.$d->Hits.'</td> 
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule('dokumen/edit/'.$d->ID_Dokumen))).'</td>	
																</tr>  '; 
															}
														}
													?>
