								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('menu/kategori_add')); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_Kategori).'<i></i></label></td>
																	<td>'.$d->Nm_Kategori.'</td>
																	<td>'.$d->Deskripsi.'</td>
																	<td>'.form_hidden('ID_Kategori[]',$d->ID_Kategori).form_input('Sort_Order[]',set_value('Sort_Order[]',$d->Sort_Order),'class="form-control"').'</td>
																	<td align="center">'.($d->Show_Title ? label('success','Yes') : label('error','No')).'</td>
																	<td align="center">'.icon_status($d->Status).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule('menu/kategori_edit/'.$d->ID_Kategori))).'</td>	
																</tr>  '; 
															}
														}
													?>
