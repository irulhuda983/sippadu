...
class Setting extends CI_Controller {
	...
	public function config ($idsetting=0)
	{
		...
		$d['PageAkses'] = PageAkses(28,$DataAkses);
		...
		if($this->input->post('simpan')){
			...
			$this->mod_setting->save_config($idsetting,$filename);
		}
		
		if($this->input->post('op_setting') && $this->input->post('op_setting') != 'no'){
			$url = site_url(fmodule('setting/config')).'/'.$this->input->post('op_setting');
			redirect($url);
		}
		...
		$d['title'] = $title = lang('Setting');
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'setting';
		$d['menuactive'] = 'setting_config';
		$d['op_setting'] = $this->mod_setting->op_config();
		$d['idsetting'] = $idsetting;
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $this->mod_setting->get_config($idsetting);
