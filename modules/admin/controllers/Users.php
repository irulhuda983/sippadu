...
class Users extends CI_Controller {
	...
	public function add()
	{
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url('apps'));
	...
	public function group($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(1,$DataAkses);
		...
		if($this->input->post('delete')){
			$a = $this->mod_users->delete_users_group();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_users->enable_users_group();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_users->disable_users_group();
		}
		
		$d['title'] = $title = 'Users Group Manager';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'users';
		$d['menuactive'] = 'users_group';
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_users->table_users_group($limit,$offset);
		$d['tot'] = $tot = $this->mod_users->table_users_group()->num_rows();
		
		$config['base_url'] = site_url(fmodule('users/group'));
		$config['total_rows'] = $tot;
		$config['per_page'] = $limit;
		$config['uri_segment'] = 3;
		$config['first_link'] = lang('First');
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('users/users_group',$d);
	...
	public function group_add()
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_users->insert_users_group();
		}
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url('apps'));
		$this->breadcrumb->append_crumb('Users Group', site_url(fmodule('users/group')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('users/users_group_add',$d);
	}
	...
	public function group_edit($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_users->update_users_group($id);
		}
		
		$d['title'] = $title = 'Ubah Data';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'users';
		$d['menuactive'] = 'users_group';
		$d['data'] = $this->mod_users->data_users_group($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url('apps'));
		$this->breadcrumb->append_crumb('Users Group', site_url(fmodule('users/group')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('users/users_group_edit',$d);
	}
	
	public function level($offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(1,$DataAkses);
		...
		if($this->input->post('delete')){
			$a = $this->mod_users->delete_users_level();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_users->enable_users_level();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_users->disable_users_level();
		}
		
		$d['title'] = $title = 'Users Level Manager';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'users';
		$d['menuactive'] = 'users_level';
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_users->table_users_level($limit,$offset);
		$d['tot'] = $tot = $this->mod_users->table_users_level()->num_rows();
		
		$config['base_url'] = site_url(fmodule('users/level'));
		$config['total_rows'] = $tot;
		$config['per_page'] = $limit;
		$config['uri_segment'] = 3;
		$config['first_link'] = lang('First');
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('users/users_level',$d);
	...
	public function level_add()
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_users->insert_users_level();
		}
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url('apps'));
		$this->breadcrumb->append_crumb('Users Level', site_url(fmodule('users/level')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('users/users_level_add',$d);
	}
	...
	public function level_edit($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_users->update_users_level($id);
		}
		...
		$d['menuopen'] = 'users';
		$d['menuactive'] = 'users_level';
		$d['data'] = $this->mod_users->data_users_level($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url('apps'));
		$this->breadcrumb->append_crumb('Users Level', site_url(fmodule('users/level')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('users/users_level_edit',$d);
	}
