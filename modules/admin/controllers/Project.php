...
class Project extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('mod_project','mod_all'));
	...
	public function index($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_project->delete_project();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_project->enable_project();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_project->disable_project();
		}
		...
		//$d['menuopen2'] = 'web_project';
		$d['menuactive'] = $this->class_page;
		
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_project->tabel_project($limit,$offset);
		$d['tot'] = $tot = $this->mod_project->tabel_project()->num_rows();
	...
	public function add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_project->insert_project($filename,$raw_name,$ori_ext);
		}
		...
		$d['op_project_kategori'] = $this->mod_project->op_project_kategori();
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($this->title_page, site_url(fmodule($this->class_page)));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('project/project_add',$d);
	}
	...
	public function edit($id=0)
	{
		...
		if($this->input->post('submit')){
			...
			if ($this->input->post('hapus_image') || $_FILES['userfile']['name']){
				$dataimg= $this->mod_project->data_project($id);
			...
			$a = $this->mod_project->update_project($id,$filename,$raw_name,$ori_ext);
		}
		...
		$d['menuactive'] = $this->class_page;
		$d['op_project_kategori'] = $this->mod_project->op_project_kategori();
		$d['data'] = $this->mod_project->data_project($id);
		$d['slc_op_project_kategori'] = $this->mod_project->slc_op_project_kategori($id);
		$d['slc_tags'] = $this->mod_project->slc_project_tags($id);
	...
	public function kategori($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_project->delete_project_kategori();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_project->enable_project_kategori();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_project->disable_project_kategori();
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_project->tabel_project_kategori($limit,$offset);
		$d['tot'] = $tot = $this->mod_project->tabel_project_kategori()->num_rows();
	...
	public function kategori_add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_project->insert_project_kategori();
		}
	...
	public function kategori_edit($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_project->update_project_kategori($id);
		}
		...
		$d['menuopen'] = $this->class_page;
		$d['menuactive'] = $this->class_page.'_kategori';
		$d['data'] = $this->mod_project->data_project_kategori($id);
