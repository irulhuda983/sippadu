...
class Agenda extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('mod_agenda','mod_all'));
	...
	public function index($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_agenda->delete_agenda();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_agenda->enable_agenda();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_agenda->disable_agenda();
		}
		...
		//$d['menuopen2'] = 'web_agenda';
		$d['menuactive'] = $this->class_page;
		
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_agenda->tabel_agenda($limit,$offset);
		$d['tot'] = $tot = $this->mod_agenda->tabel_agenda()->num_rows();
	...
	public function add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_agenda->insert_agenda($filename,$raw_name,$ori_ext);
		}
		...
		$d['op_agenda_kategori'] = $this->mod_agenda->op_agenda_kategori();
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($this->title_page, site_url(fmodule($this->class_page)));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('agenda/agenda_add',$d);
	}
	...
	public function edit($id=0)
	{
		...
		if($this->input->post('submit')){
			...
			if ($this->input->post('hapus_image') || $_FILES['userfile']['name']){
				$dataimg= $this->mod_agenda->data_agenda($id);
			...
			$a = $this->mod_agenda->update_agenda($id,$filename,$raw_name,$ori_ext);
		}
		...
		$d['menuactive'] = $this->class_page;
		$d['op_agenda_kategori'] = $this->mod_agenda->op_agenda_kategori();
		$d['data'] = $this->mod_agenda->data_agenda($id);
		$d['slc_op_agenda_kategori'] = $this->mod_agenda->slc_op_agenda_kategori($id);
		$d['slc_tags'] = $this->mod_agenda->slc_agenda_tags($id);
	...
	public function kategori($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_agenda->delete_agenda_kategori();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_agenda->enable_agenda_kategori();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_agenda->disable_agenda_kategori();
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_agenda->tabel_agenda_kategori($limit,$offset);
		$d['tot'] = $tot = $this->mod_agenda->tabel_agenda_kategori()->num_rows();
	...
	public function kategori_add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_agenda->insert_agenda_kategori();
		}
	...
	public function kategori_edit($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_agenda->update_agenda_kategori($id);
		}
		...
		$d['menuopen'] = $this->class_page;
		$d['menuactive'] = $this->class_page.'_kategori';
		$d['data'] = $this->mod_agenda->data_agenda_kategori($id);
