...
class Grafik extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('mod_grafik','mod_all'));
	...
	public function index($offset=0)
	{
		...
		$d['PageAkses'] = PageAkses(15,$DataAkses);
		
		if($this->input->post('delete')){
			$a = $this->mod_grafik->delete_grafik();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_grafik->enable_grafik();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_grafik->disable_grafik();
		}
		
		$d['title'] = $title = 'Grafik';
		...
		$d['data'] = $data = $this->mod_grafik->tabel_grafik($limit,$offset);
		$d['tot'] = $tot = $this->mod_grafik->tabel_grafik()->num_rows();
		
		$config['base_url'] = site_url(fmodule('grafik/index'));
	...
	public function add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_grafik->insert_grafik($filename,$raw_name,$ori_ext);
		}
		
		$d['title'] = $title = 'Tambah Grafik';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		$d['menuopen2'] = 'grafik';
		$d['menuactive'] = 'grafik';
		$d['op_grafik_jenis'] = $this->mod_grafik->op_grafik_jenis();
		$d['op_grafik_kategori'] = $this->mod_grafik->op_grafik_kategori();
		$d['slc_op_grafik_kategori'] = $this->mod_grafik->slc_op_grafik_kategori($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('Grafik', site_url(fmodule('grafik')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_add',$d);
	...
	public function edit($id=0)
	{
		...
		if($this->input->post('submit')){
			...
			if ($this->input->post('hapus_image') || $_FILES['userfile']['name']){
				$dataimg= $this->mod_grafik->data_grafik($id);
			...
			$a = $this->mod_grafik->update_grafik($id,$filename,$raw_name,$ori_ext);
		}
		
		$d['title'] = $title = 'Edit Grafik';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		$d['menuopen2'] = 'grafik';
		$d['menuactive'] = 'grafik';
		$d['data'] = $this->mod_grafik->data_grafik($id);
		$d['op_grafik_jenis'] = $this->mod_grafik->op_grafik_jenis();
		$d['op_grafik_kategori'] = $this->mod_grafik->op_grafik_kategori();
		$d['slc_op_grafik_kategori'] = $this->mod_grafik->slc_op_grafik_kategori($id);
		$d['slc_tags'] = $this->mod_grafik->slc_grafik_tags($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('Grafik', site_url(fmodule('grafik')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_edit',$d);
		
	}
	
	public function data($id=0,$offset=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(15,$DataAkses);
		...
		if($this->input->post('delete')){
			$a = $this->mod_grafik->delete_grafik_data();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_grafik->enable_grafik_data();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_grafik->disable_grafik_data();
		}
		elseif($this->input->post('save')){
			$a = $this->mod_grafik->save_grafik_data();
		}
		
		$Nm_Grafik = $this->mod_grafik->get_grafik($id);
		$d['title'] = $title = 'Grafik: '.$Nm_Grafik;
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		$d['menuopen2'] = 'grafik';
		$d['menuactive'] = 'grafik';
		...
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_grafik->tabel_grafik_data($id,$limit,$offset);
		$d['tot'] = $tot = $this->mod_grafik->tabel_grafik_data($id)->num_rows();
		
		$config['base_url'] = site_url(fmodule('grafik/data'));
		$config['total_rows'] = $tot;
		$config['per_page'] = $limit;
		$config['uri_segment'] = 3;
		$config['first_link'] = lang('First');
	...
	public function data_add($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_grafik->insert_grafik_data($id);
		}
		
		
		$Nm_Grafik = $this->mod_grafik->get_grafik($id);
		$d['title'] = $title = 'Tambah Grafik Data: '.$Nm_Grafik;
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		$d['menuopen2'] = 'grafik';
		$d['menuactive'] = 'grafik';
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('Grafik', site_url(fmodule('grafik')));
		$this->breadcrumb->append_crumb($Nm_Grafik, site_url(fmodule('grafik/data/'.$id)));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_data_add',$d);
	}
	
	public function data_edit($kategori=0,$id=0)
	{
		$d = $this->mod_all->load();
		$d['DataAkses']	= $DataAkses = DataAkses($this->session->userdata('userid'));
		$d['PageAkses'] = PageAkses(15,$DataAkses);
		...
		if($this->input->post('submit')){
			$a = $this->mod_grafik->update_grafik_data($id);
		}
		
		$Nm_Grafik = $this->mod_grafik->get_grafik($kategori);
		$d['title'] = $title = 'Edit Grafik Data: '.$Nm_Grafik;
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		$d['menuopen2'] = 'grafik';
		$d['menuactive'] = 'grafik';
		$d['data'] = $this->mod_grafik->data_grafik_data($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('Grafik', site_url(fmodule('grafik')));
		$this->breadcrumb->append_crumb($Nm_Grafik, site_url(fmodule('grafik/data/'.$kategori)));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_data_edit',$d);
	}
	...
	public function kategori($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_grafik->delete_grafik_kategori();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_grafik->enable_grafik_kategori();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_grafik->disable_grafik_kategori();
		}
		
		$d['title'] = $title = 'Kategori Grafik';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		//$d['menuopen2'] = 'web_grafik';
		$d['menuactive'] = 'grafik_kategori';
		
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_grafik->tabel_grafik_kategori($limit,$offset);
		$d['tot'] = $tot = $this->mod_grafik->tabel_grafik_kategori()->num_rows();
		
		$config['base_url'] = site_url(fmodule('grafik/kategori'));
		$config['total_rows'] = $tot;
		$config['per_page'] = $limit;
		$config['uri_segment'] = 3;
		$config['first_link'] = lang('First');
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_kategori',$d);
	...
	public function kategori_add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_grafik->insert_grafik_kategori();
		}
		
		$d['title'] = $title = 'Tambah Kategori Grafik';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		//$d['menuopen2'] = 'web_grafik';
		$d['menuactive'] = 'grafik_kategori';
		...
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('Kategori Grafik', site_url(fmodule('grafik/kategori')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_kategori_add',$d);
	...
	public function kategori_edit($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_grafik->update_grafik_kategori($id);
		}
		
		$d['title'] = $title = 'Edit Kategori Grafik';
		$d['site_title'] = site_title($title);
		$d['menuopen'] = 'grafik';
		$d['menuopen2'] = 'web_grafik';
		$d['menuactive'] = 'grafik_kategori';
		$d['data'] = $this->mod_grafik->data_grafik_kategori($id);
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb('Kategori Grafik', site_url(fmodule('grafik/kategori')));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('grafik/grafik_kategori_edit',$d);
