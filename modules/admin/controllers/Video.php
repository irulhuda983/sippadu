...
class Video extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('mod_video','mod_all'));
	...
	public function index($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_video->delete_video();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_video->enable_video();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_video->disable_video();
		}
		...
		//$d['menuopen2'] = 'web_video';
		$d['menuactive'] = $this->class_page;
		
		
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_video->tabel_video($limit,$offset);
		$d['tot'] = $tot = $this->mod_video->tabel_video()->num_rows();
	...
	public function add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_video->insert_video($filename,$raw_name,$ori_ext);
		}
		...
		$d['op_video_kategori'] = $this->mod_video->op_video_kategori();
		$d['op_video_source'] = $this->mod_video->op_video_source();
		
		$this->breadcrumb->append_crumb(lang('Dashboard'), site_url(fmodule()));
		$this->breadcrumb->append_crumb($this->title_page, site_url(fmodule($this->class_page)));
		$this->breadcrumb->append_crumb($title, '/');
		$this->load->view('video/video_add',$d);
	}
	...
	public function edit($id=0)
	{
		...
		if($this->input->post('submit')){
			...
			if ($this->input->post('hapus_image') || $_FILES['userfile']['name']){
				$dataimg= $this->mod_video->data_video($id);
			...
			$a = $this->mod_video->update_video($id,$filename,$raw_name,$ori_ext);
		}
		...
		$d['op_video_kategori'] = $this->mod_video->op_video_kategori();
		$d['op_video_source'] = $this->mod_video->op_video_source();
		$d['data'] = $this->mod_video->data_video($id);
		$d['slc_op_video_kategori'] = $this->mod_video->slc_op_video_kategori($id);
		$d['slc_tags'] = $this->mod_video->slc_video_tags($id);
	...
	public function kategori($offset=0)
	{
		...
		if($this->input->post('delete')){
			$a = $this->mod_video->delete_video_kategori();
		}
		elseif($this->input->post('enable')){
			$a = $this->mod_video->enable_video_kategori();
		}
		elseif($this->input->post('disable')){
			$a = $this->mod_video->disable_video_kategori();
		}
		...
		$d['limit'] = $limit = config('limit');
		$d['data'] = $data = $this->mod_video->tabel_video_kategori($limit,$offset);
		$d['tot'] = $tot = $this->mod_video->tabel_video_kategori()->num_rows();
	...
	public function kategori_add()
	{
		...
		if($this->input->post('submit')){
			...
			$a = $this->mod_video->insert_video_kategori();
		}
	...
	public function kategori_edit($id=0)
	{
		...
		if($this->input->post('submit')){
			$a = $this->mod_video->update_video_kategori($id);
		}
		...
		$d['menuopen'] = $this->class_page;
		$d['menuactive'] = $this->class_page.'_kategori';
		$d['data'] = $this->mod_video->data_video_kategori($id);
