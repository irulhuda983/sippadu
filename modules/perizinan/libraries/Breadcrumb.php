...
class Breadcrumb {
	...
	private $_tag_open 		= '<div class="page-bar"><ul class="page-breadcrumb">';
	private $_tag_close 	= '</ul></div>';
	 ...
	 * @return	void
	 */		
	function prepend_crumb($title, $href, $active='')
	{
		// no title or href provided
		if (!$title OR !$href) return;
		
		// add to start
		array_unshift($this->breadcrumbs, array('title' => $title, 'href' => $href, 'active' => $active));
	}
	...
	function output()
	{
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				// if last element
				if (array_keys($this->breadcrumbs) == $key OR $crumb['href'] == '/' OR $crumb['href'] == '') {
					$output .= $this->_li_open . '&nbsp;&nbsp;&nbsp;'.$crumb['title'].'' . $this->_li_close;
					
				// else add link and divider
				} else {
					$output .= $this->_li_open.'<a href="' . $crumb['href'] . '">' . $crumb['title'] . '</a>'.$this->_li_close;
				}
			}
