...
class Mod_siup extends CI_Model {
	...
	function insert_siup(){
		...
		$this->form_validation->set_rules('op_kota2','Kota','is_natural_no_zero');
		...
		if($this->form_validation->run() == TRUE){
			...
			$qc_last_id = $this->db->select_max('ID','ID')->get('tbsiup');
			$last_id_izin = 1;
			...
			$qc_last_reg = $this->db->select_max('No_Reg','No_Reg')->where('YEAR(TGL_BUAT)',TimeNow('Y'))->get('tbsiup');
			$last_reg_izin = 1;
			...
			$in4['ID'] = $last_id_izin;
			$in4['IDSIUP'] = str_pad($last_reg_izin,5,0,STR_PAD_LEFT).'/SIUP/'.TimeNow('Y');
			$in4['NO_MOHON'] = $this->input->post('no_mohon');
			$in4['TGL_MOHON'] = db_time($this->input->post('tgl_mohon'));
			$in4['IDPELANGGAN'] = $idpelanggan;
			$in4['IDPERUSAHAAN'] = $idperusahaan;
			$in4['JENIS_IZIN'] = $jenis_izin = $this->input->post('jenis_izin');
			$in4['URUTAN'] = $this->input->post('urutan');
			$in4['TGL_REF'] = db_time($this->input->post('tgl_sk_lama'));
			$in4['NO_REF'] = $this->input->post('no_sk_lama');
			$in4['ID_REF'] = 0;
			$in4['TGL_MOHON'] = db_time(TimeNow());
			$in4['STATUS_PENGURUS'] = $this->input->post('op_pengurus_permohonan');
			...
			$in4['JENIS_PERUSAHAAN'] = '02';
			$in4['TGL_BUAT'] = TimeNow('Y-m-d H:i:s');
			$in4['IDAKSES'] = $this->session->userdata('sippadu_userfullname');
			$in4['Status'] = 1;
			
			$a = $this->db->insert('tbsiup',$in4);
			...
			if($data_dok->num_rows() > 0){ 
				foreach($data_dok->result() as $dok){ 
					...
					if ($_FILES['userfile_'.$Kd_Dok]['name'] != ''){
						$config['upload_path'] 	= $upload_path = './assets/'.config('theme').'/files/berkas/siup/';
