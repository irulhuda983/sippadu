...
function picprof($id=0){
	...
	$h = imgprofil($image_raw.'_thumb'.$image_ext,'user',config('main_site'));
	return $h;
}
...
function op_izin(){
	$CI =& get_instance();
	$a = $CI->db->order_by('Kd_Izin','asc')->where('Reg_Online',1)->where('Status',1)->get('tb_izin');
	$d = array();
	if($a->num_rows() > 0){
		$d['0']	= '- Silakan Pilih Jenis Izin -';
		foreach($a->result() as $aa){
			$d[$aa->Kd_Izin] = $aa->Nm_Izin;
		}
	}
	else{
		$d['0']	= 'Tidak ada data';
