			<?php
			if($this->config->item('tampilan_layout_informasi') == 'tabel'){
			?>
			...
			elseif($this->config->item('tampilan_layout_informasi') == 'list'){
			?>
						...
						<li>
							<section class="featuredArticle">
								<div class="col-12-m">
									<?php 
									if($data_informasi->num_rows() > 0){
										foreach($data_informasi->result() as $db){
											echo '
											<a href="'.autolink_informasi($db->ID_Info).'"  class="thumbnail thumbLong">
												<figure>
													<span class="image thumbCrop-latestnews">'.imgsrc($db->Image_Raw.'_original'.$db->Image_Ext,'informasi',config('main_site')).'</span>
													<figcaption>
														<span class="tag">'.informasi_kategori($db->ID_Info).'</span>
