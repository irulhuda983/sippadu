...
(function( app ){
	...
	app.photoVariants = {
		 "Embed Photo": { width: 500, height: 333},
		 "Widescreen": { width: 1350, height: 759}
	};
	...
	app.competitionTeams = {
		16: [ 385, 336, 337, 339, 343, 344, 381, 387, 355, 356, 383, 358 ],
		17: [ 386, 332, 334, 341, 345, 346, 347, 351, 352, 354, 357, 360 ]
	};
	app.statCompSeasons = { // Competition Season Ids that we have opta data for
		U21: [ 59, 90, 96 ],
		U18: [ 60, 89 ]
	};
	...
	app.paths = [
		{
			...
			crm: "//crm.dev.platform.pulselive.com/premierleague/eventgroup/en/12",
            ...
            formApi: 'https://form-api.dev.platform.pulselive.com/premierleague/forms/',
		...
		},
		{
			...
			crm: "//crm.test.platform.pulselive.com/premierleague/eventgroup/en/31",
			cdn: "/resources/ver/",
            ...
            formApi: 'https://form-api.test.platform.pulselive.com/premierleague/forms/',
			playerImagePath : "//platform-static-files.s3.amazonaws.com/premierleague/photos/players/",
			fantasyUserApi: "https://plusers.ismgames.com",
			fantasyAppId: 'pl-web',
			redirectUri: 'https://testpl.ismgames.com/',
		...
		},
		{
			...
			api : "//api.test.platform.pulselive.com",
			cms: "//api.test.platform.pulselive.com/content/PremierLeague",
			crm: "//crm.test.platform.pulselive.com/premierleague/eventgroup/en/12",
		...
		},
		{
			...
			crm: "//crm.dev.platform.pulselive.com/premierleague/eventgroup/en/12",
			canary : "https://api.canary.platform.pulselive.com/production/",
			formApi: 'https://form-api.platform.pulselive.com/premierleague/forms/',
			playerImagePath : "//platform-static-files.s3.amazonaws.com/premierleague/photos/players/",
	...
	if( app.environment.label == "development" ){
		app.newSeason = {
			'FIRST': 72
		};
	}
	else if( app.environment.label == "test" ){
		app.newSeason = {
			'FIRST': 75
		};
	}
	...
	app.apiPath = {
		...
		'cms': {
			...
			'article' : app.environment.cms + '/playlist/en/',
			'events': app.environment.crm + '/events/'
        }
	...
	app.googleMapsKey = "AIzaSyCsRrqInJdQQ15BlJK8QQTiJM1h0sx6SKY";
	app.financialStandingsInfoClubIds = [1, 6, 11, 25, 43];
...
( function ( app, common ) {
    ...
    // gets the data of the competitions passed in the parameter and returns as a flattened array
    var getPrimaryComps = function(comps) {
        var primaryComps = [];
        comps.forEach(function(comp) {
            if (app.filterData.comps.primary[comp]) {
                primaryComps.push(app.filterData.comps.primary[comp]);
            }
        });
        return [].concat.apply([], primaryComps); // flatten array
    };
        ...
        app.filterGroups.matchHighlights = [
            {
                ...
                dataIdPath: [ "id" ],
                data: getPrimaryComps(['FIRST', 'U21']),
                dependencies: [],
                urlParameter: "co"
            },
            {
                ...
                listTransform: function ( list ) {
                    return list.sort(function (a, b) {
                        var prevClubName = a.name.toLowerCase();
                        var nextClubName = b.name.toLowerCase();

                        if ( prevClubName < nextClubName ) {
                            return -1;
                        }
        ...
        app.filterGroups.editorial = [
            {
                ...
                dataNamePath: [ "description" ],
                dataIdPath: [ "id" ],
                data: getPrimaryComps(['FIRST', 'U21', 'U18']),
                dependencies: [],
                urlParameter: "co"
            },
            {
        ...
        app.filterGroups.referee = [
            {
                ...
                label: "filterbyseason",
                // 2017/18 changeover
                defaultOptionId: app.newSeason.FIRST,
                dataUrl: app.environment.api + "/football/competitions/" + app.filterData.comps.primary.FIRST.id + "/compseasons?page=0&pageSize=100",
                dataListPath: [ "content" ],
                dataNamePath: [ "label" ],
                dataIdPath: [ "id" ],
        ...
        app.filterGroups.seasonawards = [
            {
                ...
                label: "filterbyseason",
                // 2017/18 changeover
                defaultOptionId: app.lastSeason.FIRST,
                dataUrl: app.environment.api + "/football/competitions/" + app.filterData.comps.primary.FIRST.id + "/compseasons?page=0&pageSize=100",
                dataListPath: [ "content" ],
                dataNamePath: [ "label" ],
                dataIdPath: [ "id" ],
                ...
                'player-pl2': [
                    {
                        ...
                        label: "filterbyseason",
                        // 2017/18 changeover
                        defaultOptionId: app.lastSeason.U21,
                        data: [
                                {
                                "label": "2016/17",
                                "id": 59
                ...
                'player-u18': [
                    {
                        ...
                        label: "filterbyseason",
                        // 2017/18 changeover
                        defaultOptionId: app.lastSeason.U18,
                        data: [
                                {
                                "label": "2016/17",
                                "id": 60
        ...
        app.filterGroups.stats =
            {
                'player': [
                    ...
                    },
                    {
                        ...
                        dataNamePath: [ "label" ],
                        dataIdPath: [ "id" ],
                        listTransform: function ( list, element, dependency ) {
                            var compId = parseInt( dependency.dependencyMap[ "FOOTBALL_COMPETITION" ].id );
                            dependency.config.defaultOption = "All Seasons";

                            app.filterData.comps.primary.U21.forEach(function(u21comp) {
                                if (u21comp.id === compId) {
                                    dependency.config.defaultOption = "";
                                    list = list.filter( function( listItem ) {
                                        return (app.statCompSeasons.U21.indexOf( listItem.id ) > -1);
                                    });
                                }
                            });

                            if (app.filterData.comps.primary.U18.id === compId) {
                                dependency.config.defaultOption = "";
                                list = list.filter( function( listItem ) {
                                    return (app.statCompSeasons.U18.indexOf( listItem.id ) > -1);
                                });
                            }
            ...
            "FIRST": [
                ...
                },
                {
                    ...
                    dependancyCheck: function ( dependencies ) {
                        // 2017/18 changeover
                        var comp = "FOOTBALL_COMPETITION";
                        var season = "compSeasons";
                        if ( dependencies[ comp ] && dependencies[ comp ].id == app.filterData.comps.primary.FIRST.id ) {

                            if (dependencies[ season ] && dependencies[ season ].id != app.newSeason.FIRST ) {
                                return true;
                            }
            ...
            "U21": [
                {
                    name: "FOOTBALL_COMPETITION",
                    label: "filterbycompetition",
                    defaultId: app.filterData.comps.primary.U21[0].id,
                    dataListPath: [],
                    dataNamePath: [ "description" ],
                    dataIdPath: [ "id" ],
                    dependencies: [],
                ...
                },
                {
                    ...
                    dataListPath: [ "content" ],
                    dataNamePath: [ "label" ],
                    defaultId: app.filterData.getCurrentCompSeasonId(app.filterData.comps.primary.U21[0].id),
                    dataIdPath: [ "id" ],
                    dependencies: [ "FOOTBALL_COMPETITION" ],
                    urlParameter: "se"
                }
            ...
            "U21": [
                {
                    ...
                    name: "comps",
                    label: "filterbycompetition",
                    defaultOptionId: app.filterData.comps.primary.U21[0].id,
                    dataListPath: [ "content" ],
                    dataNamePath: [ "description" ],
                    dataIdPath: [ "id" ],
                    data: app.filterData.comps.pl.U21.comps,
...
(function( app, core ){
	...
	var getData = function(){
		var setDefaultData = function( data ) {
			...
			/*
			 * Primary competitions for each professional level
			 * e.g. in app.competitions.FIRST it contains ID 2 which is Champions League. We just want PL related competitions here.
			 */
			defaultData.primary = {
				"FIRST": defaultData.FIRST[ 0 ], // 1
				"U21": [ defaultData.U21[ 0 ], defaultData.U21[ 1 ], defaultData.U21[ 2 ] ], // 17, 18, 11
				"U18": defaultData.U18[ 0 ] // 8
			};
		};
...
(function( app ){
	...
	app.widgetDeps = function( container ){
		...
		var scriptPrefix = "http://localhost/themes/" + theme + "/resources/ver/scripts/pl/";
		var scriptSuffix = app.environment.label === "production" ? ".js" : ".js";
		...
		function addDependancy( dep ){
			if( deps.indexOf( dep ) < 0 ){
				...
				//alert(scriptPrefix + dep + scriptSuffix);
			}
		}
	};
...
(function ( app, common ) {
    ...
    window.onload = function () {
        ...
        //Polyfill any object-fit images with class "poly-object-fit" 
        objectFitImages('poly-object-fit');
    };
