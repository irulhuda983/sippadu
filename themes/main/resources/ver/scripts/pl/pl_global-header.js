...
(function ( app, common, core ) {
	...
	app.mainNavigation = function( element, config ) {
		...
		_self.subNavContainer = document.querySelector( '.subNav' );
		...
		var moreNavWithUL = {
			navWrap: _self.subNavContainer,
			moreLabel: "More"
		};

		_self.moreNav = new PULSE.ui.moreNav( moreNavWithUL );
	...
	app.mainNavigation.prototype.setListeners = function() {
		...
		core.event.listenForMultiple( _self.mobileMenuExpander, [ 'keypress', 'click' ], function() {
			...
			var isModalActive = document.querySelector( 'body .plModal.open');
			if(isModalActive !== null) {
				app.modalsController.closeAll();
			}
