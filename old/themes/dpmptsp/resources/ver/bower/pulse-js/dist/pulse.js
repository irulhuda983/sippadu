...
(function( core ){
	...
	core.data.manager = (function(){
		...
		var Poll = function( config ) {
			...
			_self.formData = config.formData || false;
			...
			var request = function(){
				if( !checkConstant() || !_self.data  ){
					core.data.request( _self.url, _self.method, notify, _self.target, _self.jsonpCallback, _self.data, _self.headers, _self.type, _self.formData );
				}
				else{
		...
		var Subscriber = function( config, poll ) {
			...
			_self.request = function(){
				core.data.request( poll.url, "GET", _self.callback, _self.target, _self.jsonpCallback, _self.data, _self.headers, _self.type, _self.formData );
			};
		};
		...
		var Add = function( config ) {
				...
				if( !polls[config.url] ) {
					polls[config.url] = new Poll({
						...
						formData: config.formData || false
					});
				}
...
(function( core ){
	 ...
	 */
	core.data.request = function( url, method, callback, target, jsonpCallback, data, headers, type, formData ){
		var config = {
			url: url,
			method: method || "GET",
			target: target,
		...
		if ( jsonpCallback ){
			...
			if( !containsContentTypeHeader && !config.formData ) {
			...
			if(config.method === "POST") {
				xhttp.send(config.formData ? config.data : formattedParams(config.data));
			} else {
...
( function( core ) {
	...
	core.dom.isElementInView = function ( element, fullyInView ) {
        var pageTop = typeof window.scrollY === 'undefined' ? window.pageYOffset : window.scrollY;
...
(function( core ){
    ...
    core.style.outerWidth = function( el ) {
        ...
        var marginLeft = style.marginLeft === 'auto' ? 0 : parseInt(style.marginLeft, 10);
        var marginRight = style.marginRight === 'auto' ? 0 : parseInt(style.marginRight, 10);
...
(function( ui, core ){
	...
	var bindInstance = function( instance, _self ){
        ...
        instance.activator.setAttribute( 'data-modal-active', true );
	...
	var buildItems = function( _self ){
		Array.prototype.map.call( _self.config.activators, _self.createInstance.bind( _self ) );
	};
    ...
    /**
     * Allows retroactive binding of new DOM elements as activators for the UI modal
     * @param {DOMElement} el the new DOM element to add as a trigger for the modal
     */
    ui.modal.prototype.createInstance = function( el ) {
        var _self = this;
