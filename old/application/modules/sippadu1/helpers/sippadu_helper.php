...
function desa($id=0){
	...
	if($a->num_rows() > 0){
		$aa = $a->row();
		if($id == '3522160001' || $id == '3522160004' || $id == '3522160005' || $id == '3522160006' || $id == '3522160007' || $id == '3522160008' || $id == '3522160009' || $id == '3522160010' || $id == '3522160015' || $id == '3522160016' || $id == '3522160018'){
			$addstr = 'Kelurahan';
		}
		$h = $addstr.' '.$aa->NAMA_DESA;
	}
	return $h;
}
...
function format_parameter($id_izin=0,$parameter=0,$string=array(),$replace=array()){
	$CI =& get_instance();
	$db = $CI->load->database('mysql',TRUE);
	$a = $db->where('Kd_Izin',$id_izin)->where('Param',$parameter)->get('tb_parameter');
	$h = '';
	if($a->num_rows() > 0){
		$aa = $a->first_row();
		if(is_array($string)){
...
function itkes($x=0,$type=0){
	...
	switch($type.$x){
		...
		case '02':
			...
			break;
		case '11':
			$h = 'SURAT IJIN KERJA BIDAN (SIKB)';
			break;
		case '12':
			$h = 'SURAT IJIN KERJA PERAWAT (SIKP)';
			break;
		case '21':
			$h = 'Surat Ijin Kerja Bidan (SIKB)';
			break;
