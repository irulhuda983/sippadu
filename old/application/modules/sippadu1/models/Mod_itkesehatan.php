...
class Mod_itkesehatan extends CI_Model {
	...
	function insert_itkesehatan_fo(){
		...
		$this->form_validation->set_rules('kategori_izin','Kategori Izin','');
		...
		if($this->form_validation->run() == TRUE){
			...
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_id,5,0,STR_PAD_LEFT).'/ITKES/'.TimeNow('Y');
	...
	function get_id_sk($idizin=0){
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->get('tbitkesehatan');
			$dd = $d->first_row();
