								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" name="add" value="Registrasi Izin" onclick="window.location.href='<?php echo site_url(fmodule('tdup/fo_data')); ?>'"/>
												<input class="btn btn-cyan" type="button" name="add" value="Refresh" onclick="window.location.href='<?php echo current_url(); ?>'"/>
												<input class="btn btn-greensea" type="button" name="add" value="Izin Belum Diproses" onclick="window.location.href='<?php echo site_url(fmodule('tdup/fo')); ?>'"/>
												<input class="btn btn-dutch" type="button" name="add" value="Izin Telah Diproses" onclick="window.location.href='<?php echo site_url(fmodule('tdup/fo_all')); ?>'"/>
														...
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																...
																<tr> 
																	...
																	<td align="center">'.anchor_popup_label(site_url(fmodule('tdup/tt/'.$d->ID)),'Cetak Tanda Terima','info',$popup).'</td>	
																	<td align="center">'.anchor_label(site_url(fmodule('tdup/fo_edit/'.$d->ID)),'Ubah','success').'</td>	
																</tr>  ';
															}
														}
													?>
