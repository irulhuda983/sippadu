...
function berita_kategori($id=0){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$a = $db->query('SELECT a.*,b.Nm_Kategori FROM ci_berita_log a LEFT JOIN ci_berita_kategori b ON a.ID_Kategori = b.ID_Kategori WHERE a.ID_Berita = '.$id);
		$k = array();
		$h = '';
		if($a->num_rows() > 0){
			foreach($a->result() as $aa){
...
function level_menu_option($parent=0,$level=0,$selected=0,$except=0,$category=''){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$data = $db->order_by('Sort_Order','asc')->order_by('Nm_Menu','asc')->where('ID_Parent',$parent)->like('ID_Kategori',$category)->where('ID_Menu !=',$except)->get('ci_menu');
	$h = '';
	if($data->num_rows() > 0){
...
function level_menu_tabel($category=0,$parent=0,$level=0){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$data = $db->order_by('Sort_Order','asc')->order_by('Nm_Menu','asc')->where('ID_Kategori',$category)->where('ID_Parent',$parent)->get('ci_menu');
	$h = '';
	if($data->num_rows() > 0){
		foreach($data->result() as $d){
			$h .= '<tr>
...
function cek_level_menu($id=0,$count=0){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$data = $db->where('ID_Menu',$id)->get('ci_menu');
	$h = $count;
	if($data->num_rows() > 0){
		$d = $data->row();
...
function icon_dropdown_menu($id=0,$attribute=''){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$data = $db->where('ID_Parent',$id)->where('Status',1)->get('ci_menu');
	$h = '';
	if($data->num_rows() > 0){
		$h = $attribute;
	}
...
function child_menu($id=0){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$data = $db->where('ID_Parent',$id)->get('ci_menu');
	$h = '';
	if($data->num_rows() > 0){
...
function autolink_berita($id=0){
	...
	$db_group = $CI->db->groupname;
	$db = $CI->load->database($db_group,TRUE);
	$data = $db->where('ID_Berita',$id)->get('ci_berita');
