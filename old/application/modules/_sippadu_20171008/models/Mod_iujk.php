...
class Mod_iujk extends CI_Model {
	...
	function get_tbl_iujk_fo($status=0,$limit=0,$offset=0){
		...
		FROM
		tbiujk
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbiujk.ID_Pelanggan WHERE tbiujk.Flow_FO = 1 and tbiujk.Flow_BO = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_iujk_kabid($status=0,$limit=0,$offset=0){
		...
		FROM
		tbiujk
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbiujk.ID_Pelanggan WHERE tbiujk.Flow_FO = 1 and tbiujk.Flow_BO = 1 and tbiujk.Flow_Kabid = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_iujk_kadin($status=0,$limit=0,$offset=0){
		...
		FROM
		tbiujk
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbiujk.ID_Pelanggan WHERE tbiujk.Flow_FO = 1 and tbiujk.Flow_BO = 1 and tbiujk.Flow_Kabid = 1 and tbiujk.Flow_Kadin = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_iujk_tu($status=0,$limit=0,$offset=0){
		...
		FROM
		tbiujk
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbiujk.ID_Pelanggan WHERE tbiujk.Flow_FO = 1 and tbiujk.Flow_BO = 1 and tbiujk.Flow_Kabid = 1 and tbiujk.Flow_Kadin = 1 and tbiujk.Flow_TU = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function get_tbl_iujk_cetak($status=0,$limit=0,$offset=0){
		...
		FROM
		tbiujk
		INNER JOIN tbpelanggan ON tbpelanggan.IDPELANGGAN = tbiujk.ID_Pelanggan WHERE tbiujk.Flow_FO = 1 and tbiujk.Flow_BO = 1 and tbiujk.Flow_Kabid = 1 and tbiujk.Flow_Kadin = 1 and tbiujk.Flow_TU = 1 and tbiujk.Flow_Serah = "'.$status.'" '.$q_where.' '.$q_order.' '.$q_limit.'');
		return $a;
	}
	...
	function insert_iujk_fo(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$in2['ID'] = $last_id;
			$in2['ID_Reg'] = str_pad($last_id,5,0,STR_PAD_LEFT).'/IUJK/'.TimeNow('Y');
	...
	function op_kategori_iujk(){
		...
		$h['2'] = 'Pelaksanaan';
		$h['3'] = 'Pengawasan';
		$h['4'] = 'Perencaan, Pelaksaan dan Pengawasan';
		return $h;
	}
	...
	function get_id_sk($idizin=0){
		...
		if($h=='' or $this->input->post('submit')){
			$d = $this->db->select_max('ID_SK','ID_SK')->get('tbiujk');
			$dd = $d->first_row();
