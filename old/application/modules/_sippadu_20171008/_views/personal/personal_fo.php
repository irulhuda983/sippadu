								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" name="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('personal/fo_add')); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->IDPELANGGAN).'<i></i></label></td>
																	<td>'.$d->IDPELANGGAN.'</td>
																	<td>'.$d->NM_PELANGGAN.'</td>
																	<td>'.$d->ALAMATPELANGGAN.'</td>
																	<td>'.$d->NOKITAS.'</td> 
																	<td align="center">'.anchor_label(site_url(fmodule('personal/fo_edit/'.$d->IDPELANGGAN)),'Ubah','success').'</td>	
																</tr>  '; 
															}
														}
													?>
