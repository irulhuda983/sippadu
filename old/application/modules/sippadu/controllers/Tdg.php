...
class Tdg extends MX_Controller {
	...
	public function serah_win($idizin=0)
	{
		...
		$this->load->view('tdg/tdg_serah_win',$d);
	}
	
	public function serah_ajax()
	{
		$d['data'] = $this->mod_tdg->data_serah_ajax();
		$this->load->view('tdg/tdg_serah_ajax',$d);
	}
	...
	public function fo_data($offset=0)
	{
		...
		$config['base_url'] = $url_parent = site_url(fmodule('tdg/fo_data'));
		$config['total_rows'] = $tot;
		$config['per_page'] = $limit;
		$config['uri_segment'] = 4;
		$config['first_link'] = 'First';
	...
	public function tu_edit($idtdg=0)
	{
		...
		$this->breadcrumb->append_crumb('Register TDG', site_url(fmodule('tdg/tu')));
		//$this->breadcrumb->append_crumb('Registrasi Izin', site_url(fmodule('tdg/fo_data')));
		$this->breadcrumb->append_crumb($title);
		$this->load->view('tdg/tdg_edit',$d);
	}
