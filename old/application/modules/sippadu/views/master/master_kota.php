								...
								<?php echo form_open('',array('name' => 'form', 'id' => 'form')); ?>
									<div class="tile-widget">
										...
										<div class="row">
											...
											<div class="col-sm-8">
												<input class="btn btn-blue" type="button" id="add" value="Tambah Data" onclick="window.location.href='<?php echo site_url(fmodule('master/kota_add/'.$provinsi)); ?>'"/>
													...
													<?php
														if($data->num_rows() > 0){
															foreach($data->result() as $d){
																echo '
																<tr>                                    
																	<td class="center"><label class="checkbox checkbox-custom-alt checkbox-custom-sm m-0">'.form_checkbox('chk[]',$d->ID_KOTA).'<i></i></label></td>
																	<td>'.$d->ID_PROVINSI.'</td>
																	<td>'.$d->NAMA_PROVINSI.'</td>
																	<td>'.$d->ID_KOTA.'</td>
																	<td>'.anchor(fmodule('master/kecamatan/'.$d->ID_KOTA),$d->NAMA_KOTA).'</td>
																	<td align="center">'.icon_status($d->STATUS).'</td>
																	<td align="center">'.icon_edit(site_url(fmodule('master/kota_edit/'.$d->ID_KOTA))).'</td>	
																</tr>  '; 
															}
														}
													?>
